...
| `research/problem_statement.md` requirement index | Must be amended (component-wide living doc) to reflect: renamed external-component records, new `FeatReq`/`CompReq`, and a dated `## Changelog` entry pointing at this cycle, per `rules-score-update`'s "Keeping it current". |
