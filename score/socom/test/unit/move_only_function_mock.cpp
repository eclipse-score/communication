...
using Return_value_function = move_only_function<std::unique_ptr<int>(int), 128U, alignof(std::max_align_t)>;
...
TEST(move_only_function_mock_test, value_return_works)
{
    ...
    const auto result = mock.as_function()(7);
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(*result, 11);
}

void call_callback(Value_consuming_function callback, const int value)
{
    callback(value);
}
