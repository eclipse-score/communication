...
using Atomic_getter = const std::atomic<bool>& (Bridge_data::*)() const;
...
struct Bridge_param
{
    ...
    explicit Bridge_param(const Bridge_param_tuple& param_tuple)
    ...
    {
        const auto sequence =
            0 == requests_before_bridge_creation ? Bridge_data::bridge_then_expect : Bridge_data::expect_then_bridge;
        return sequence;
    }

    Bridge_param with_clients_already_created() const
    {
        return Bridge_param{std::make_tuple(
            get_total_requests(), 0, num_bridges, order, delete_and_recreate_bridges, delete_and_recreate_requests)};
    }

    Bridge_data::Expect expect_or_nothing(const Bridge_data::Expect& expect) const
    {
        return 0 != get_total_requests() ? expect : Bridge_data::nothing;
    }
};

std::string readable_test_names_bridge(const TestParamInfo<Bridge_param_tuple>& param_info)
{
    const auto param = Bridge_param{param_info.param};
    std::stringstream ss;
...
template <typename T>
void pop_empty(std::vector<T>& v, const std::function<bool()>& get_atom, const Destruction_order& order)
{
    while (!v.empty())
    {
...
Bridges create_bridges(const Bridge_param& param, const Bridge_data::Expect& expect, Connector_factory& factory)
{
    auto result = Bridges{param.num_bridges};
    for (auto& bridge : result)
    {
        bridge = std::make_unique<Bridge_data>(param.get_sequence(), param.expect_or_nothing(expect), factory);
    }
    return result;
}

bool request_service_destroyed(const Bridges& bridges)
{
    const auto request_find_service_destroyed = [](const Bridges::value_type& bridge) {
...
std::function<bool()> create_get_destroyed(const Bridges& bridges,
                                           const std::function<bool(const Bridges&)>& get_destroyed_fun)
{
    const auto get_destroyed = [&bridges, get_destroyed_fun]() {
        return get_destroyed_fun(bridges);
    };
    return get_destroyed;
}

void check_construction_of_callback(const Bridge_param& param,
                                    const Bridges& bridges,
                                    const Atomic_getter& getter,
                                    const std::function<void(const Bridge_data&)>& post_check_action)
{
...
void clean_bridges(Bridges& bridges, const bool clients_destroyed)
{
...
void expect_callbacks(Bridges& bridges, const Bridge_data::Expect& expect, const Connector_factory& connector_factory)
{
    for (auto& bridge : bridges)
    {
...
template <typename T>
void clean_test(std::vector<T>& requests,
                Bridges& bridges,
                const Bridge_param& param,
                const std::function<bool()>& get_destroyed)
{
    if (Destruction_order::requests_first == param.order)
    {
...
void recreate_bridges(const Bridge_param& param,
                      const Bridge_data::Expect& expect,
                      const Atomic_getter& getter,
                      Bridges& bridges,
                      ...
                      const std::function<void(const Bridge_data&)>& post_check_action)
{
    ...
    {
        clean_bridges(bridges, false);
        append(bridges, create_bridges(param.with_clients_already_created(), expect, connector_factory));
        check_construction_of_callback(param, bridges, getter, post_check_action);
    }
}

template <typename REQUESTS, typename REQUESTSCREATOR>
void recreate_requests(REQUESTS& requests,
                       Bridges& bridges,
...
void bridge_test_template(CREATEREQUESTS const& create_requests,
                          const Bridge_param& param,
                          Connector_factory& connector_factory,
                          const Bridge_data::Expect& expect,
                          const std::function<bool(const Bridges&)>& get_destroyed_fun,
                          ...
                          const std::function<void(const Bridge_data&)>& post_check_action)
{
    ...
    const auto get_destroyed = create_get_destroyed(bridges, get_destroyed_fun);
    ...
    recreate_requests(
        clients, bridges, connector_factory, param, expect, getter, get_destroyed, create_requests, post_check_action);
...
{
    const Runtime::Uptr rt = create_runtime();
...
{
  protected:
    const std::vector<Service_instance> input_find_result{Service_instance{"first instance", Literal_tag{}},
                                                          Service_instance{"second instance", Literal_tag{}},
                                                          connector_factory.get_instance()};
    const Service_instance expected_find_result{connector_factory.get_instance()};
...
{
    const std::multiset<Service_instance> expected_set{expected.cbegin(), expected.cend()};
    const std::multiset<Service_instance> input_set{arg.cbegin(), arg.cend()};
    return (input_set == expected_set);
}

TEST_F(RuntimeTest, RegisterServiceBridgeWithIncompleteCallbacksReturnsCallbacksMissing)
{
    const auto callback_missing = score::MakeUnexpected(Construction_error::callback_missing);

    const auto registration = connector_factory.register_service_bridge(Bridge_identity::make(*this), nullptr);
    EXPECT_EQ(registration, callback_missing);
}

TEST_F(RuntimeTest, RegisterServiceBridgeWitCallbacksReturnsRegistration)
{
    const score::Result<Service_bridge_registration> registration =
        connector_factory.register_service_bridge(Bridge_identity::make(*this), rsf_mock.AsStdFunction());
...
{
    Bridge_data bridge{Bridge_data::bridge_then_expect, Bridge_data::request_service_function, connector_factory};

    const Client_data client{connector_factory, Client_data::no_connect};
...
TEST_F(RuntimeTest, BridgeDoesNotReceiveRequestServiceFunctionCallForKnownService)
{
    ...
    const Client_data client{connector_factory};
    EXPECT_FALSE(bridge.get_request_find_service_created());
}

TEST_F(RuntimeTest, BridgeCreatesServerConnectorInRequestServiceFunctionCallbackAtClientConnectorCreation)
{
    std::optional<Server_data> server;
    const auto create_server = [this, &server](const Service_interface_definition& /*configuration*/,
                                               const Service_instance& /*instance*/) {
        server.emplace(connector_factory);
    };

    Bridge_data bridge{Bridge_data::bridge_then_expect, Bridge_data::nothing, connector_factory};
    bridge.expect_request_find_service(
        connector_factory.get_configuration(), connector_factory.get_instance(), create_server);

    Service_state_change_callback_mock state_change_callback;
    {
        const InSequence is;
        EXPECT_CALL(state_change_callback, Call(_, Service_state::available, connector_factory.get_configuration()));
    }
    const Client_data client{connector_factory, Client_data::might_connect, state_change_callback.as_function()};
}

TEST_F(RuntimeTest, ConstructDuplicateReturnsDuplicateServiceError)
{
    Server_connector_callbacks_mock callbacks;
    const auto scd = connector_factory.create_server_connector_with_result(callbacks);
    ASSERT_TRUE(scd.has_value());

    Server_connector_callbacks_mock callbacks_2;
    const auto scd_2 = connector_factory.create_server_connector_with_result(callbacks_2);
    const score::Result<Disabled_server_connector::Uptr> duplicate_service =
...
TEST_F(RuntimeTest, ConstructDuplicateMultipleTimesReturnsDuplicateServiceError)
{
    ...
    const auto scd_3 = connector_factory.create_server_connector_with_result(callbacks_3);
...
TEST_F(RuntimeTest, CreatingServerConnectorDeletingItAndRecreatingReturnsValidServerConnector)
{
    Server_connector_callbacks_mock callbacks;
    {
        ...
        const score::Result<Disabled_server_connector::Uptr> duplicate_service =
            score::MakeUnexpected(Construction_error::duplicate_service);
        EXPECT_EQ(duplicate_service, connector_factory.create_server_connector_with_result(callbacks_2));
    }
...
{
    const auto create_requests = [this](size_t num_requests) {
        return Client_data::create_clients(connector_factory, num_requests, Client_data::no_connect);
    };

    const auto post_check = [](const Bridge_data& /*bridge*/) {};

    bridge_test_template(create_requests,
                         m_param,
                         connector_factory,
                         Bridge_data::request_service_function,
                         request_service_destroyed,
                         &Bridge_data::get_request_find_service_created,
                         post_check);
}

INSTANTIATE_TEST_SUITE_P(RequestsAfterBridgeCreation,
                         RuntimeBridgeTest,
                         Combine(Values(0),
                                 Values(0, 1),
                                 Values(1, 10),
                                 Values(Destruction_order::requests_first, Destruction_order::bridges_first),
                                 Bool(),
                                 Bool()),
