...
{
    return std::make_shared<std::tuple<Method_result, Method_result>>(Method_result{std::move(result)},
                                                                      Method_result{std::move(expected_result)});
}

TEST(Method_type_test, method_result_equality_and_inequality_operators)
{
    // Create Method_result objects
    const auto ar = Method_result{Application_return{}};
    const auto ae = Method_result{Application_error{}};
    const auto me = Method_result{Error::runtime_error_request_rejected};
    const auto ae_code = Method_result{Application_error{10}};
    const auto ae_payload = Method_result{Application_error{make_vector_payload(Vector_buffer{10})}};
    ...
    const auto vector_payload1 = make_vector_payload(2, make_vector_buffer(1U, 2U, 3U, 4U, 5U));
    const auto vector_payload1_copy = make_vector_payload(2, make_vector_buffer(1U, 2U, 3U, 4U, 5U));
    const auto vector_payload2 = make_vector_payload(2, make_vector_buffer(9U, 8U, 7U, 6U, 5U));
    const auto vector_payload3 = make_vector_payload(2, make_vector_buffer(1U, 2U, 7U, 6U, 5U));
    const auto vector_payload4 = make_vector_payload(2, make_vector_buffer(9U, 8U, 3U, 4U, 5U));
    const auto ar1 = Application_return{clone_payload(vector_payload1)};
    const auto ar1_copy = Application_return{clone_payload(vector_payload1_copy)};
    const auto ar2 = Application_return{clone_payload(vector_payload1)};
    const auto ar3 = Application_return{clone_payload(vector_payload3)};
    const auto ar4 = Application_return{clone_payload(vector_payload4)};
...
class MethodCallTest : public SingleConnectionTest, public WithParamInterface<Input_expected>
{
  protected:
    ...
    const Method_result& input_result{std::get<0>(*GetParam())};
    const Method_result& expected_result{std::get<1>(*GetParam())};
};

TEST_P(MethodCallTest, ClientSendsFireAndForgetMethodCallToServer)
{
    const auto& call_received = server.expect_and_respond_method_calls(1, method_id, input_data(), input_result);
...
INSTANTIATE_TEST_SUITE_P(MethodCallTestInstance,
                         MethodCallTest,
                         Values(create_results(Ar{}, Ar{}),
                                ...
                                create_results(Me::runtime_error_request_rejected,
                                               Me::runtime_error_request_rejected)));
...
TEST_F(MethodCallCredentialsTest, ClientCallsMethodServerReceivesCustomClientCredentials)
{
    ...
    Server_data server{connector_factory,
                       connector_factory.get_configuration(),
                       connector_factory.get_instance(),
                       default_credentials};
    Client_data client{
        connector_factory, connector_factory.get_configuration(), connector_factory.get_instance(), client_credentials};
...
TEST_F(AllocateMethodPayloadTest, AllocateMethodPayloadWithoutConnectedServerReturnsServiceNotAvailableError)
{
...
TEST_F(AllocateMethodPayloadTest, AllocateMethodPayloadWithConnectedServerReturnsPayload)
{
    ...
    const auto& expect_method_payload_allocation =
...
TEST_F(AllocateMethodPayloadTest, ServerReceivesReplyPayloadAndRespondsToMethodCall)
{
    ...
    const auto* const ptr = reply_payload->data().data();
    ...
    EXPECT_CALL(method_reply_callback_mock, Call(_)).WillOnce([ptr](const auto& mr) {
        ASSERT_TRUE(std::holds_alternative<Application_return>(mr));
        const auto& ar = std::get<Application_return>(mr);
        EXPECT_EQ(ptr, ar.payload.data().data());
    });
    reply_data->reply(Method_result{Application_return{Payload{std::move(*reply_data->get_reply_payload())}}});
}
