...
class MultipleRuntimesTest : public ::testing::Test
{
  protected:
    ...
    Connector_factory factory0 = Connector_factory{Service_interface_identifier{test_service_id, {1, 0}},
                                                   to_num_of_methods(2U),
                                                   to_num_of_events(3U),
                                                   Service_instance{test_instance_id}};

    Connector_factory factory1 = Connector_factory{Service_interface_identifier{test_service_id, {2, 3}},
                                                   to_num_of_methods(2U),
                                                   to_num_of_events(3U),
                                                   Service_instance{test_instance_id}};

    const Payload real_payload = make_vector_payload(make_vector_buffer(1U, 2U, 3U, 4U));
    const Payload more_payload =
        make_vector_payload(make_vector_buffer(1U, 2U, 3U, 4U, 5U, 3U, 2U, 4U, 3U, 2U, 4U, 2U, 4U, 5U, 5U, 3U));
};
...
TEST_F(MultipleRuntimesTest, MultipleServerAndClientsAreCreatedAndLastPairExchangesAnEvent)
{
    ...
    const auto event = static_cast<Event_id>(factory0.get_num_events() / 2);

    pair1.server.expect_event_subscription(event);
    const auto sub = pair1.client.create_event_subscription(event);
...
TEST_F(MultipleRuntimesTest, MultipleServerAndClientsAreCreatedAndFirstPairExchangesAnEvent)
{
    ...
    const auto sub = pair0.client.create_event_subscription(event);
...
TEST_F(MultipleRuntimesTest, MultipleServerAndClientsAreSendingEvents)
{
    ...
    const auto event0 = static_cast<Event_id>(factory0.get_num_events() / 2);

    pair0.server.expect_event_subscription(event0);
    const auto sub0 = pair0.client.create_event_subscription(event0);
    ...
    const auto sub1 = pair1.client.create_event_subscription(event1);
...
{
    const auto method0 = static_cast<Event_id>(factory0.get_num_methods() / 2);
    const auto result0 = Method_result{Application_return{clone_payload(real_payload)}};
...
{
    const auto method1 = Method_id{1};
    const auto result1 = Method_result{Application_return{clone_payload(real_payload)}};
...
TEST_F(MultipleRuntimesTest, MultipleServerAndClientsAndBothHaveMethodCalls)
{
    ...
    const auto method1 = Method_id{1};
    const auto result1 = Method_result{Application_return{clone_payload(more_payload)}};
