...
class ServerConnectorTest : public SingleConnectionTest
{
  protected:
    ...
    const score::Result<Disabled_server_connector::Uptr> callback_missing =
...
TEST_F(ServerConnectorTest, ConfigurationAndInstanceIsAsExpected)
{
    ...
    const auto& service_cfg_disabled = scd->get_configuration();
    ...
    const auto validate_configuration = [this](const auto& sce) {
        const auto& service_cfg_enabled = sce.get_configuration();
    ...
    const auto validate_service_instance = [this](const auto& sce) {
    ...
    EXPECT_CALL(callbacks, on_event_subscription_change(_, _, _))
        .Times(2)
        .WillRepeatedly(
            [&validate_configuration, &validate_service_instance](auto& sce, auto /* id */, auto /* state */) {
                validate_configuration(sce);
                validate_service_instance(sce);
            });
    Client_data client0{connector_factory};
    const auto subscription = client0.create_event_subscription(event_id);
}
...
{
    auto scd =
        connector_factory.create_server_connector_with_result(Optional_reference<Server_connector_callbacks_mock>{});
...
TEST_F(ServerConnectorTest, WhenCallbackMissingCreationReturnsCallbackMissing)
{
    std::array<Disabled_server_connector::Callbacks, 4> server_callbacks_array = {
        Disabled_server_connector::Callbacks{nullptr, esccb.as_function(), eruc.as_function(), mpacb.as_function()},
        Disabled_server_connector::Callbacks{mccb.as_function(), nullptr, eruc.as_function(), mpacb.as_function()},
        Disabled_server_connector::Callbacks{mccb.as_function(), esccb.as_function(), nullptr, mpacb.as_function()},
        Disabled_server_connector::Callbacks{mccb.as_function(), esccb.as_function(), eruc.as_function(), nullptr},
    };

    for (auto& server_callbacks : server_callbacks_array)
    {
        auto scd = connector_factory.create_server_connector_with_result(std::move(server_callbacks));
...
class ServerConnectorDeathTest : public ServerConnectorTest
{
  protected:
    ...
    Client_data client0;
    const ::std::uint32_t deadlock_detected_by_destroying_server_connector_inside_callback_id = 0x50000004U;
    const std::string expected_message{"SOCom error: A callback causes the Enabled_server_connector"};

    ServerConnectorDeathTest() : server{connector_factory.create_and_enable(callbacks)}, client0{connector_factory}
    {
        // triggered by server self destruction.
        client0.expect_service_state_change(Service_state::not_available);
    }

    ~ServerConnectorDeathTest() override
    {
        server.reset();
    }
};
...
TEST_F(ServerConnectorDeathTest, ServerDeletionByOnEventSubscriptionChangeResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        const auto self_destruct =
            [this](Enabled_server_connector& /* server */, Event_id /* id */, Event_state /* state */) {
                server.reset();
            };
       ...
       ServerDeletionByOnEventSubscriptionChangeAtUnsubscriptionResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        ...
        const auto subscription = client0.create_event_subscription(event_id);
        EXPECT_CALL(callbacks, on_event_subscription_change(_, _, Event_state::unsubscribed)).WillOnce(self_destruct);
    };
    EXPECT_DEATH(el_failure(), expected_message);
}

TEST_F(ServerConnectorDeathTest, ServerDeletionByOnEventSubscriptionChangeWithInitialDataResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        const auto self_destruct = [this](Enabled_server_connector& /* server */, Event_id /* id */) {
            server.reset();
        };
...
TEST_F(ServerConnectorDeathTest, ServerDeletionByOnEventUpdateRequestResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        EXPECT_CALL(callbacks, on_event_subscription_change(_, _, _));
        const auto subscription = client0.create_event_subscription(event_id);
...
{
    const auto el_failure = [this]() {
        const auto self_destruct = [this](Enabled_server_connector& /*server*/,
                                          Method_id /*method_id*/,
...
{
    const auto el_failure = [this]() {
        const auto empty_reply = [](Enabled_server_connector& /*server*/,
                                    Method_id /*method_id*/,
                                    const Payload& /*payload*/,
                                    Method_call_reply_data_opt reply) {
        ...
        Method_reply_callback reset_server = [this](const Method_result& /*mr*/) {
            server.reset();
        };
...
class ServerConnectorGetEventModeTest : public SingleConnectionTest
{
  protected:
    Server_data server{connector_factory};
    Client_data client0{connector_factory};
    const std::atomic_bool& wait_subscribed{
...
class ServerConnectorOutOfBoundsTest : public SingleConnectionTest
{
  protected:
    ...
    score::Result<void> out_of_range = score::MakeUnexpected(Server_connector_error::logic_error_id_out_of_range);
};
...
{
    const auto out_of_range = score::MakeUnexpected(Server_connector_error::logic_error_id_out_of_range);
