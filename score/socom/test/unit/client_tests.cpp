...
{
bool operator==(const Client_connector& /*lhs*/, const Client_connector& /*rhs*/)
{
    ADD_FAILURE();
    return false;
}
...
Client_connector::Uptr create_connected_client_with_event_subscription(Connector_factory& connector_factory,
                                                                       Server_data& server,
...
std::function<void(const Client_connector&, Service_state, Service_interface_definition)> delete_client_state_change(
    std::unique_ptr<Client_connector>& client0)
{
    return [&client0](const auto& /* client_connector */, auto /* state */, const auto& /* configuration */) {
        client0.reset();
    };
}

std::function<void(const Client_connector& /*cc*/, const Event_id /*eid*/, const Payload& /*pl*/)> delete_client(
    std::unique_ptr<Client_connector>& client0)
{
    const auto delete_client = [&client0](
                                   const Client_connector& /*cc*/, const Event_id /*eid*/, const Payload& /*pl*/) {
        client0.reset();
    };
...
struct Test_setup
{
    ...
    const Service_interface_definition service_interface_configuration;
    const Test_method_ids test_method_ids;
    const Test_event_ids test_event_ids;
};
...
// an error
std::ostream& operator<<(std::ostream& out, const Test_setup& /*ignored*/)
{
    return out;
}
...
const Method_result application_return{Application_return{}};

const auto service_interface =
    Service_interface_identifier{std::string_view{"TestInterface1"}, Service_interface_identifier::Version{1U, 2U}};
const auto service_instance = Service_instance{std::string_view{"TestInstance1"}};

const Server_service_interface_definition server_configuration{service_interface,
                                                               to_num_of_methods(2U),
                                                               to_num_of_events(3U)};

const Test_setup full_client_configuration_setup{test_values::server_configuration,
                                                 {{0U, 0U}, {1U, 1U}},
                                                 {{0U, 0U}, {1U, 1U}, {2U, 2U}}};

const Service_interface_definition no_client_configuration{service_interface};

const Test_setup no_client_configuration_setup{no_client_configuration,
                                               {{0U, 0U}, {1U, 1U}},
                                               {{0U, 0U}, {1U, 1U}, {2U, 2U}}};

// method variations
const Server_service_interface_definition method_without_event_configuration{service_interface,
                                                                             to_num_of_methods(2U),
                                                                             to_num_of_events(0U)};

const Test_setup method_without_event_configuration_setup{method_without_event_configuration, {{0U, 0U}, {1U, 1U}}, {}};

const Service_interface_definition method_subset_client_configuration{service_interface,
                                                                      to_num_of_methods(1U),
                                                                      to_num_of_events(3U)};

const Test_setup method_subset_client_configuration_setup{method_subset_client_configuration,
                                                          {{0U, 0U}},
                                                          {{0U, 0U}, {1U, 1U}, {2U, 2U}}};

// event variations
const Server_service_interface_definition event_without_method_configuration{service_interface,
                                                                             to_num_of_methods(0U),
                                                                             to_num_of_events(3U)};

const Test_setup event_without_method_configuration_setup{event_without_method_configuration,
                                                          {},
                                                          {{0U, 0U}, {1U, 1U}, {2U, 2U}}};

const Service_interface_definition event_subset_client_configuration{service_interface,
                                                                     to_num_of_methods(2U),
                                                                     to_num_of_events(2U)};

const Test_setup event_subset_client_configuration_setup{event_subset_client_configuration,
                                                         {{0U, 0U}, {1U, 1U}},
                                                         {{0U, 0U}, {1U, 1U}}};

const auto setups = Values(full_client_configuration_setup,
                           no_client_configuration_setup,
                           method_without_event_configuration_setup,
                           method_subset_client_configuration_setup,
                           event_without_method_configuration_setup,
                           event_subset_client_configuration_setup);
...
class ConstructionClientConnectorTest : public ::testing::TestWithParam<test_values::Test_setup>
{
  protected:
    ...
    const score::Result<Client_connector::Uptr> callback_missing =
...
TEST_P(ConstructionClientConnectorTest, ConstructCallbackMissing)
{
    ...
    for (auto& callbacks : input)
    {
        auto cc = connector_factory.create_client_connector_with_result(
            GetParam().service_interface_configuration, test_values::service_instance, std::move(callbacks));
...
INSTANTIATE_TEST_SUITE_P(ConstructionClientConnectorTestInstance, ConstructionClientConnectorTest, test_values::setups);
...
class UnconnectedClientConnectorTest : public ::testing::TestWithParam<test_values::Test_setup>
{
  protected:
    ...
    Client_connector_callbacks_mock callbacks;
    Client_connector::Uptr cc{connector_factory.create_client_connector(GetParam().service_interface_configuration,
                                                                        test_values::service_instance,
                                                                        callbacks)};

    const score::Result<void> service_not_available = score::MakeUnexpected(Error::runtime_error_service_not_available);
};
...
TEST_P(UnconnectedClientConnectorTest, CreationOfSecondClientConnectorReturnsDuplicateError)
{
    ...
    auto second_cc = connector_factory.create_client_connector_with_result(
        GetParam().service_interface_configuration, test_values::service_instance, create_client_callbacks(callbacks));
...
{
    for (const auto& input : GetParam().test_event_ids)
    {
...
TEST_P(UnconnectedClientConnectorTest, UnsubscribeEventIgnoreSubscribedEventReturnsServiceNotAvailable)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        EXPECT_EQ(service_not_available, cc->subscribe_event(input.client_id, Event_mode::update));
...
TEST_P(UnconnectedClientConnectorTest, RequestEventUpdateIgnoreSubscribedEventReturnsServiceNotAvailable)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        EXPECT_EQ(service_not_available, cc->subscribe_event(input.client_id, Event_mode::update));
...
TEST_P(UnconnectedClientConnectorTest, CallMethodReturnsServiceNotAvailable)
{
    ...
    const auto no_connection_method = score::MakeUnexpected(Error::runtime_error_service_not_available);

    for (const auto& input : GetParam().test_method_ids)
    {
        const auto result =
            cc->call_method(input.client_id, empty_payload(), Method_call_reply_data{mrcb.as_function(), std::nullopt});

        EXPECT_EQ(result, no_connection_method);
    }
}

INSTANTIATE_TEST_SUITE_P(UnconnectedClientConnectorTestInstance, UnconnectedClientConnectorTest, test_values::setups);

class UnavailableServerConnectorClientConnectorTest : public TestWithParam<test_values::Test_setup>
{
  protected:
    Connector_factory connector_factory{test_values::server_configuration, test_values::service_instance};

    Server_connector_callbacks_mock server_callbacks;
    Disabled_server_connector::Uptr server{connector_factory.create_server_connector(server_callbacks)};
    ...
    const Event_state subscribed{Event_state::subscribed};
};
...
TEST_P(UnavailableServerConnectorClientConnectorTest, UnsubscribeEventIgnoreSubscribedEventReturnsServiceNotAvailable)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        EXPECT_EQ(service_not_available, cc->subscribe_event(input.client_id, Event_mode::update));
...
TEST_P(UnavailableServerConnectorClientConnectorTest, RequestEventUpdateIgnoreSubscribedEventReturnsServiceNotAvailable)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        EXPECT_EQ(service_not_available, cc->subscribe_event(input.client_id, Event_mode::update));
...
INSTANTIATE_TEST_SUITE_P(UnavailableServerConnectorClientConnectorTestInstance,
                         UnavailableServerConnectorClientConnectorTest,
                         test_values::setups);
...
class ConnectedClientConnectorTest : public ::testing::TestWithParam<test_values::Test_setup>
{
  protected:
    ...
    std::unique_ptr<Client_data> client_owner{std::make_unique<Client_data>(connector_factory,
                                                                            GetParam().service_interface_configuration,
                                                                            test_values::service_instance)};
...
TEST_P(ConnectedClientConnectorTest, CallMethodWithoutReply)
{
    for (const auto& input : GetParam().test_method_ids)
    {
        auto reply_future = server.expect_and_return_method_call(input.server_id, test_values::real_payload);
...
TEST_P(ConnectedClientConnectorTest, CallMethodWithReply)
{
    for (const auto& input : GetParam().test_method_ids)
    {
        ...
        const auto& wait_reply =
            client.expect_and_call_method(input.client_id, test_values::real_payload, test_values::application_return);
...
TEST_P(ConnectedClientConnectorTest, DestroyClientConnectorPendingMethod)
{
    ...
    const auto& input = GetParam().test_method_ids.at(0);

    auto reply_callback_future = server.expect_and_return_method_call(input.server_id, empty_payload());
...
TEST_P(ConnectedClientConnectorTest, RequestEventUpdate)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        ...
        const auto& wait_update_request = server.expect_update_event_request(input.server_id);
...
TEST_P(ConnectedClientConnectorTest, OnEventUpdate)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        ...
        const auto& wait_event_update = client.expect_event_update(input.client_id, test_values::real_payload);
...
TEST_P(ConnectedClientConnectorTest, OnRequestedEventUpdate)
{
    for (const auto& input : GetParam().test_event_ids)
    {
        ...
        const auto& wait_request_update = server.expect_update_event_request(input.server_id);
        client.request_event_update(input.client_id);
        wait_for_atomics(wait_request_update);

        const auto& wait_event_update =
...
INSTANTIATE_TEST_SUITE_P(ConnectedClientConnectorTestInstance, ConnectedClientConnectorTest, test_values::setups);

class ClientConnectorTest : public SingleConnectionTest
{
  protected:
    const score::Result<void> not_available = score::MakeUnexpected(Error::runtime_error_service_not_available);
};
...
TEST_F(ClientConnectorTest, ConfigurationAndInstanceIsAsExpected)
{
    ...
    auto validate_configuration = [this](const Client_connector& cc) {
        const auto& service_cfg = cc.get_configuration();
    ...
    auto validate_service_instance = [this](const Client_connector& cc) {
    ...
    EXPECT_CALL(callbacks, on_service_state_change(_, Service_state::available, _))
        .WillOnce([&validate_configuration, &validate_service_instance](
                      const Client_connector& cc, auto /* state */, const auto& /* cfg */) {
...
TEST_F(ClientConnectorTest, ServiceStateIsRetrievable)
{
    ...
    EXPECT_CALL(callbacks, on_service_state_change(_, Service_state::available, _))
        .WillOnce([](const Client_connector& cc, auto /* state */, const auto& /* cfg */) {
...
TEST_F(ClientConnectorTest, RetrieveServerConnectorConfig)
{
    ...
    const auto& available =
        client0.expect_service_state_change(1, Service_state::available, connector_factory.get_configuration());
...
{
    auto modified_interface_id = connector_factory.get_configuration().get_interface().id.data() + std::to_string(1);
    const auto conf = Server_service_interface_definition{
...
{
    const auto modified_instance_id = Service_instance{connector_factory.get_instance().id.data() + std::to_string(1)};
    Server_data server{connector_factory, connector_factory.get_configuration(), modified_instance_id};

    Client_data sanity_client{connector_factory, connector_factory.get_configuration(), modified_instance_id};
...
{
    const auto modified_major_interface = Service_interface_identifier{
        connector_factory.get_configuration().get_interface().id,
        {static_cast<uint16_t>(connector_factory.get_configuration().get_interface().version.major + 1),
         connector_factory.get_configuration().get_interface().version.minor}};
    const auto conf = Server_service_interface_definition{modified_major_interface,
                                                          to_num_of_methods(connector_factory.get_num_methods()),
                                                          to_num_of_events(connector_factory.get_num_events())};
    Server_data server{connector_factory, conf, connector_factory.get_instance()};
...
{
    const auto modified_minor_interface = Service_interface_identifier{
        connector_factory.get_configuration().get_interface().id,
        {connector_factory.get_configuration().get_interface().version.major,
         static_cast<uint16_t>(connector_factory.get_configuration().get_interface().version.minor - 1)}};

    const auto conf = Server_service_interface_definition{modified_minor_interface,
                                                          to_num_of_methods(connector_factory.get_num_methods()),
                                                          to_num_of_events(connector_factory.get_num_events())};
    Server_data server{connector_factory, conf, connector_factory.get_instance()};
...
TEST_F(ClientConnectorTest, OnStateChangeDoMethodCall)
{
    ...
    Client_data client0{connector_factory,
                        Client_data::might_connect,
                        [this, &available](const auto& client_connector, auto state, const auto& configuration) {
                            EXPECT_EQ(state, Service_state::available);
                            EXPECT_EQ(configuration, connector_factory.get_configuration());

                            const auto handle = client_connector.call_method(method_id, empty_payload(), {});
                            available = true;
                        }};
    ...
    EXPECT_CALL(server_callbacks, on_method_call(_, method_id, payload_eq(empty_payload()), _))
        .WillOnce(DoAll(Assign(&method_call, true), Return(ByMove(std::make_unique<Method_invocation>()))));
...
TEST_F(ClientConnectorTest, OnStateChangeRequestEventUpdate)
{
    ...
    EXPECT_CALL(server_callbacks, on_event_update_request(_, event_id)).WillOnce(Assign(&event_update_request, true));
...
TEST_F(ClientConnectorTest, GetPeerCredentialsWithoutServerReturnsServiceNotAvailable)
{
    ...
    auto check_peer_credentials = [](const Client_connector& cc, auto /* state */, const auto& /*unused*/) {
    ...
    auto no_peer_credentials = [](const Client_connector& cc, auto /* state */, const auto& /*unused*/) {
        auto credentials = cc.get_peer_credentials();
    ...
    EXPECT_CALL(callbacks, on_service_state_change(_, Service_state::available, _)).WillOnce(check_peer_credentials);
    EXPECT_CALL(callbacks, on_service_state_change(_, Service_state::not_available, _)).WillOnce(no_peer_credentials);
    const Server_data server{connector_factory};
}
...
TEST_F(ClientConnectorStartingConnectionDeathTest, ClientDeletionByOnStateChangeResultsInLoggingAndTermination)
{
    ...
    const std::string expected_message{"SOCom error: A callback causes the Client_connector"};
...
class ClientConnectorDeathTest : public ClientConnectorTest
{
  protected:
    ...
    Server_data server{connector_factory};
    std::unique_ptr<Client_connector> client0 =
        create_connected_client_with_event_subscription(connector_factory, server, cc_callbacks, event_id);
    const std::string expected_message{"SOCom error: A callback causes the Client_connector"};
...
TEST_F(ClientConnectorDeathTest, ClientDeletionByOnRequestedEventUpdateResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        ...
        EXPECT_CALL(cc_callbacks, on_requested_event_update(_, _, _)).WillOnce(delete_client(client0));
...
TEST_F(ClientConnectorDeathTest, ClientDeletionByOnMethodReplyResultsInLoggingAndTermination)
{
    const auto el_failure = [this]() {
        ...
        const auto delete_client = [this](const Method_result& /*eid*/) {
            client0.reset();
        };
        ASSERT_TRUE(
            client0->call_method(method_id, empty_payload(), Method_call_reply_data{delete_client, std::nullopt}));
...
{
  protected:
    const score::Result<void> out_of_range = score::MakeUnexpected(Error::logic_error_id_out_of_range);
};
...
TEST_F(ClientConnectorOutOfBoundsTest, SubscribeEventWithOutOfBoundsIndexReturnsOutOfRange)
{
    Server_data server{connector_factory};
    Client_connector_callbacks_mock callbacks;
    const auto client0 = connector_factory.create_and_connect(callbacks);
...
{
    const auto mr = Method_result{Application_return{}};
    ...
    const auto out_of_range_method = score::MakeUnexpected(Error::logic_error_id_out_of_range);
    EXPECT_EQ(out_of_range_method,
              (client0->call_method(
                  max_method_id + 1, empty_payload(), Method_call_reply_data{mrcb.as_function(), std::nullopt})));
}

TEST_F(ClientConnectorOutOfBoundsTest, ResultCompareOperator)
{
    const auto result_value = score::Result<std::uint8_t>(42U);
    const auto result_error = score::MakeUnexpected(Error::logic_error_id_out_of_range);
...
{
    const auto result_error_void = score::MakeUnexpected(Error::logic_error_id_out_of_range);
    const auto result_value_void = score::Result<void>();
