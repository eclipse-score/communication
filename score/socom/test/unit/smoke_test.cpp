...
#include "score/socom/mock/callback_mocks.hpp"
...
{
  protected:
    Server_service_interface_definition config{Service_interface_identifier{"example.interface", Literal_tag{}, {1, 0}},
                                               to_num_of_methods(1),
                                               to_num_of_events(1)};
...
class Connection_test : public Runtime_test
{
  protected:
    void SetUp() override
    {
        ...
        auto client_connector_result = runtime->make_client_connector(config, instance, make_client_callbacks());
        auto server_connector_result = runtime->make_server_connector(config, instance, make_server_callbacks());
        ...
        server_connector = Disabled_server_connector::enable(std::move(server_connector_result.value()));
    }
...
{
    const auto client_connector_result = runtime->make_client_connector(config, instance, make_client_callbacks());
    EXPECT_TRUE(client_connector_result);
}

TEST_F(Runtime_test, server_connector_construction_works)
{
    const auto server_connector_result = runtime->make_server_connector(config, instance, make_server_callbacks());
...
TEST_F(Runtime_test, connection_setup_works)
{
    ...
    const auto enabled_server_connector = Disabled_server_connector::enable(std::move(server_connector_result.value()));
...
TEST_F(Connection_test, client_calls_method_and_gets_response)
{
    ...
    EXPECT_CALL(m_method_call_mock, Call(_, 0, _, _, _)).WillOnce([&pointer](auto&, auto, auto, auto cb_data, auto) {
        pointer = std::move(cb_data);
        return nullptr;
    });
    const auto invocation = client_connector->call_method(
        0, empty_payload(), Method_call_reply_data{m_method_reply_mock.as_function(), std::nullopt});
