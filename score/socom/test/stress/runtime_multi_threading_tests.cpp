...
struct State_data
{
    ...
    void check_state_machine(const Service_state& state) const
    {
        ...
        EXPECT_TRUE((Service_state::not_available != *m_old_state) || (Service_state::available == state) ||
                    (Service_state::not_available == state))
            << "old_state == " << *m_old_state << ", state == " << state;
        // available -> not_available
        EXPECT_TRUE((Service_state::available != *m_old_state) || (Service_state::not_available == state))
            << "old_state == " << *m_old_state << ", state == " << state;
    }

    bool update_reached_states(const Service_state& state)
    {
        m_old_state = state;
  ...
  public:
                                           ...
                                           const Service_instance& instance)
    {
        ...
        return [this, &factory, &configuration, &instance]() {
            Client_data client{
                factory, Client_data::might_connect, configuration, instance, m_state_change_mock.as_function()};
        };
    }

    Service_state_change_callback create_callback()
    {
        return [this, data = std::make_shared<State_data>()](const auto& /*cc*/, auto state, const auto& /*conf*/) {
            const std::unique_lock<std::mutex> lock{data->m_mutex};
...
class RuntimeMultiThreadingTest : public SingleConnectionTest
{
  protected:
    ...
    Loop_function_t create_servers_thread_main(const Server_service_interface_definition& configuration,
                                               const Service_instance& instance)
    {
    ...
    {
        const auto register_bridges = [this, rsf_mock_function = rsf_mock.AsStdFunction()]() {
            const auto registration =
                connector_factory.register_service_bridge(Bridge_identity::make(*this), rsf_mock_function);
...
TEST_F(RuntimeMultiThreadingTest, CreationOfServerAndClientConnectorsHasNoRaceCondition)
{
    ...
    const auto start_clients = client.create_thread_function(connector_factory);

    multi_threaded_test_template({create_servers_thread_main(), start_clients}, [&client]() {
        return static_cast<bool>(client.is_satisfied());
    });
}
...
TEST_F(RuntimeMultiThreadingTest, CreationOfServerAndClientConnectorsCallsCallbacksInCorrectOrder)
{
    ...
    const auto start_clients = client.create_thread_function(connector_factory, client.callback_creator());
...
TEST_F(RuntimeMultiThreadingTest, BridgesWhichCreateServerConnectorsAndClientConnectorsHaveNoRaceCondition)
{
    EXPECT_CALL(rsf_mock, Call(_, _))
        .Times(AnyNumber())
        .WillRepeatedly([this](const auto& configuration, const auto& /*unused*/) {
    ...
    EXPECT_CALL(state_change_mock, Call(_, _, _)).Times(AnyNumber()).WillRepeatedly(Assign(&client_connected, true));
    const auto start_clients = [this, &state_change_mock]() {
        Client_data client{connector_factory, Client_data::might_connect, state_change_mock.as_function()};
    };

    multi_threaded_test_template({create_bridges_thread_main(), start_clients}, [&client_connected]() {
        return static_cast<bool>(client_connected);
    });
}
