...
bool always_successful()
{
    return true;
}

auto is_ready(const std::future<void>& future)
{
    return [&future]() {
        return std::future_status::ready == future.wait_for(0ms);
    };
}
...
class ClientMultiThreadingTest : public SingleConnectionTest
{
  protected:
    ...
    const std::size_t minimum_num_event_updates = 100;
    const std::size_t min_method_calls = 100;
};
...
TEST_F(ClientMultiThreadingTest, SubscribeEventIsCalledByTwoThreadsWithoutRaceConditions)
{
    ...
    const auto events_received =
        client.expect_event_updates_min_number(minimum_num_event_updates, event_id, empty_payload());

    const auto subscribe_event = [this]() {
        client.subscribe_event(event_id);
    };
    const auto update_event = [this]() {
        server.update_event(event_id, empty_payload());
    };

    multi_threaded_test_template({update_event, subscribe_event, subscribe_event}, is_ready(events_received));
...
{
    const auto unsubscribe_event = [this]() {
        client.unsubscribe_event(event_id);
    };
...
TEST_F(ClientMultiThreadingTest, RequestEventUpdateIsCalledByTwoThreadsWithoutRaceConditions)
{
    ...
    const auto events_received =
        client.expect_requested_event_updates_min_number(minimum_num_event_updates, event_id, empty_payload());

    const auto request_event_update = [this]() {
    ...
    multi_threaded_test_template({update_event, request_event_update, request_event_update}, is_ready(events_received));
...
{
    const auto method_called = server.expect_method_calls(min_method_calls, method_id, empty_payload());

    const auto call_method = [this]() {
...
{
    const auto get_peer_credentials = [this]() {
        (void)client.get_peer_credentials();
    };
