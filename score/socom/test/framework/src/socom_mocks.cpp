...
Disabled_server_connector::Callbacks create_server_callbacks(Server_connector_callbacks_mock& mock)
{
    return Disabled_server_connector::Callbacks{
        [&mock](auto& connector, auto mid, auto payload, Method_call_reply_data_opt reply_callback, auto /*cred*/) {
            return mock.on_method_call(connector, mid, std::move(payload), std::move(reply_callback));
        },
        [&mock](auto& connector, auto eid, auto state) {
            mock.on_event_subscription_change(connector, eid, state);
        },
        [&mock](auto& connector, auto eid) {
            mock.on_event_update_request(connector, eid);
        },
...
Disabled_server_connector::Callbacks create_server_callbacks(Server_connector_credentials_callbacks_mock& mock)
{
    return Disabled_server_connector::Callbacks{
        [&mock](auto& connector,
                auto mid,
                auto payload,
                Method_call_reply_data_opt reply_callback,
                const auto& credentials) {
            return mock.on_method_call(connector, mid, std::move(payload), std::move(reply_callback), credentials);
        },
        [&mock](auto& connector, auto eid, auto state) {
            mock.on_event_subscription_change(connector, eid, state);
        },
...
{
    return Client_connector::Callbacks{[&mock](const auto& connector, auto state, const auto& configuration) {
                                           mock.on_service_state_change(connector, state, configuration);
                                       },
                                       [&mock](const auto& connector, auto id, auto payload) {
                                           mock.on_event_update(connector, id, std::move(payload));
                                       },
                                       [&mock](const auto& connector, auto id, auto payload) {
                                           mock.on_requested_event_update(connector, id, std::move(payload));
                                       },
                                       [&mock](const auto& connector, auto id) {
