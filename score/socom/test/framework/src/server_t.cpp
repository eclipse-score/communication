...
Server_connector_callbacks_mock& expect_method_call(Server_connector_callbacks_mock& sc_callbacks,
                                                    ...
                                                    const Payload& expected_payload)
{
...
Server_data::Server_data(Connector_factory& factory) : m_connector{factory.create_and_enable(m_callbacks)} {}

Server_data::Server_data(Connector_factory& factory, Method_id method_id, const Payload& expected_payload)
    : m_connector{factory.create_and_enable(expect_method_call(m_callbacks, method_id, expected_payload))}
{
}
...
Server_data::Server_data(Connector_factory& factory,
    ...
    : m_connector{factory.create_and_enable(configuration, instance, m_credential_callbacks, credentials)}
{
}
...
{
    wait_for_atomics(
        m_callback_called, m_method_callback_called, m_subscribed, m_unsubscribed, m_method_payload_allocate_called);
}

Server_connector_callbacks_mock& Server_data::get_callbacks()
{
    return m_callbacks;
}
...
Enabled_server_connector& Server_data::get_connector()
{
    return *m_connector;
}
...
void Server_data::update_event(const Event_id& event_id, const Payload& payload)
{
    m_connector->update_event(event_id, clone_payload(payload));
}

void Server_data::update_requested_event(const Event_id& event_id, const Payload& payload)
{
    m_connector->update_requested_event(event_id, clone_payload(payload));
}

const std::atomic<bool>& Server_data::expect_on_event_subscription_change(
    ...
    Event_subscription_change_callback subscription_change_callback)
{
    ...
    auto callback = std::make_shared<Event_subscription_change_callback>(std::move(subscription_change_callback));
    auto call_callback = [callback](auto& sc, const auto event_id, const auto state) {
        if (!callback->empty())
        {
...
void Server_data::expect_on_event_subscription_change_nosync(const Event_id& event_id, const Event_state& state)
{
    EXPECT_CALL(m_callbacks, on_event_subscription_change(_, event_id, state));
}

const std::atomic<bool>& Server_data::expect_update_event_request(const Event_id& event_id)
{
    EXPECT_TRUE(m_callback_called);
    m_callback_called = false;
    EXPECT_CALL(m_callbacks, on_event_update_request(_, event_id)).WillOnce(Assign(&m_callback_called, true));
    return m_callback_called;
}

const std::atomic<bool>& Server_data::expect_update_event_requests(const Event_id& event_id)
{
    EXPECT_TRUE(m_callback_called);
    m_callback_called = false;
...
void Server_data::expect_and_respond_update_event_request(const Event_id& event_id, const Payload& payload)
{
    auto cloned = std::make_shared<Payload>(clone_payload(payload));
    EXPECT_CALL(m_callbacks, on_event_update_request(_, event_id))
        .WillOnce([cloned](Enabled_server_connector& connector, const Event_id& eid) {
            connector.update_requested_event(eid, clone_payload(*cloned));
        });
}

const std::atomic<bool>& Server_data::expect_method_allocate_payload(const Method_id& method_id,
                                                                     score::Result<Writable_payload> result)
{
    EXPECT_CALL(m_callbacks, on_method_call_payload_allocate(_, method_id))
        .WillOnce(DoAll(Assign(&m_method_payload_allocate_called, true), Return(ByMove(std::move(result)))));

    m_method_payload_allocate_called = false;
    return m_method_payload_allocate_called;
}

const std::atomic<bool>& Server_data::expect_and_respond_method_calls(const size_t counter,
                                                                      const Method_id& method_id,
                                                                      ...
                                                                      const Method_result& result)
{
    ...
    const auto reply = [this, &result, counter](auto& /*connector*/, auto /*mid*/, const auto& /*pl*/, const auto& cb) {
        if (cb)
        {
            cb->reply(result);
        }
...
const std::atomic<bool>& Server_data::expect_and_respond_method_call(const Method_id& method_id,
                                                                     const Payload& payload,
                                                                     const Method_result& result)
{
...
std::future<Method_call_reply_data_opt> Server_data::expect_and_return_method_call(const Method_id& method_id,
                                                                                   const Payload& payload)
{
    EXPECT_TRUE(m_method_callback_called);
    m_method_callback_called = false;
    ...
    EXPECT_CALL(m_callbacks, on_method_call(_, method_id, payload_eq(payload), _))
        .WillOnce([this, saved_callback](auto& /*connector*/, auto /*mid*/, const auto& /*pl*/, auto cb) {
            m_method_callback_called = true;
...
std::future<void> Server_data::expect_method_calls(const std::size_t& min_num,
                                                   const Method_id& method_id,
                                                   const Payload& payload)
{
    ...
    auto check_update_count = create_check_update_count(m_num_method_calls, min_num, std::move(methods_called));

    EXPECT_CALL(m_callbacks, on_method_call(_, method_id, payload_eq(payload), _))
        .WillRepeatedly(DoAll(check_update_count, InvokeWithoutArgs([]() {
                                  return nullptr;
                              })));
...
void Server_data::expect_event_subscription(const Event_id& event_id)
{
