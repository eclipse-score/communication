...
inline Num_of_events to_num_of_events(const std::size_t value) noexcept
{
    return static_cast<Num_of_events>(value);
}

inline Num_of_methods to_num_of_methods(const std::size_t value) noexcept
{
...
struct Service_interface_definition final
{
    ...
    Service_interface_definition(Service_interface_identifier sif,
    ...
    Service_interface_definition(const Service_interface_definition&) = default;
    Service_interface_definition(Service_interface_definition&&) noexcept = default;

    ~Service_interface_definition() noexcept = default;

    Service_interface_definition& operator=(const Service_interface_definition&) = delete;
...
class Server_service_interface_definition final
{
  ...
  public:
    ...
    /// \param methods Methods of the service interface.
    /// \param events Events of the service interface.
    Server_service_interface_definition(const Service_interface_identifier& sif,
                                        Num_of_methods num_of_methods,
                                        Num_of_events num_of_events);

    Server_service_interface_definition(const Server_service_interface_definition& rhs);
    ...
    Server_service_interface_definition& operator=(const Server_service_interface_definition&) = delete;
    ...
    const Service_interface_identifier& get_interface() const noexcept;
};
