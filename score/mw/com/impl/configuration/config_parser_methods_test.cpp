...
TEST_F(ConfigParserFixture, MethodIsEnabledByDefaultWhenEnabledIsNotProvided)
{
    // Given a JSON with a method without explicit enabled flag
    auto j2 = R"(
{
  "serviceTypes": [
    {
  ...
  "serviceInstances": [
    ...
    // Then the method should be enabled by default
    const auto deployments =
        config.GetServiceInstances().at(InstanceSpecifier::Create(std::string{"abc/abc/TirePressurePort"}).value());
    const auto& lola_deployment = std::get<LolaServiceInstanceDeployment>(deployments.bindingInfo_);
