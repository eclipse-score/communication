...
TYPED_TEST(ProxyEventDeathTest, DieOnUnsubscribingWhileHoldingSamplePtrs)
{
    ...
    std::optional<SamplePtr<typename Base::SampleType>> held_sample{};
...
TYPED_TEST(ProxyEventFixture, CanConstructUnboundProxy)
{
    ...
    // first)
    ProxyEvent<typename Base::SampleType> proxy_event{kEventName2, nullptr};
    // Nothing bad happens
}
...
TYPED_TEST(ProxyEventGetNewSamplesFixture, GetNewSamplesDispatchesToBinding)
{
    ...
    Base::RecordProperty("lobster-tracing", "Communication.ProxyFieldGetNewSamples");
