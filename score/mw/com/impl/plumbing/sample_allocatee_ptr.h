...
{
    static_assert(
        std::is_void<SampleType>::value || std::is_void<OtherSampleType>::value,
        "SampleAllocateePtr rebind is only supported between SampleAllocateePtr<void> (used on binding layer) and "
        "SampleAllocateePtr<NonVoidType> (used on binding independent layer), not between two non-void types.");
    this->Swap(other);
}
...
void SampleAllocateePtr<SampleType>::Swap(SampleAllocateePtr<OtherSampleType>& other) noexcept
{
    static_assert(std::is_void<SampleType>::value || std::is_void<OtherSampleType>::value,
                  "SampleAllocateePtr rebind is only supported between SampleAllocateePtr<void> (binding layer) and "
                  "SampleAllocateePtr<NonVoidType> (binding independent layer), not between two non-void types.");
    // Search for custom swap functions via ADL, and use std::swap if none are found.
    using std::swap;
    swap(internal_, other.internal_);
    swap(allocatee_guard_, other.allocatee_guard_);
