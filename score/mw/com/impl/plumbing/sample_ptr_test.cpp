...
TEST(SamplePtr, CanDereference)
{
    ...
    mock_binding::SamplePtr mock_binding_ptr{new TestSampleType(42), [](void* p) noexcept {
...
TEST(SamplePtr, CanArrowOperator)
{
    ...
    mock_binding::SamplePtr mock_binding_ptr{new Foo{}, [](void* p) noexcept {
...
{
    mock_binding::SamplePtr pointer(new TestSampleType, [](void* p) noexcept {
        auto* const int_p = static_cast<TestSampleType*>(p);
        delete int_p;
    });
