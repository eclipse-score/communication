...
{
  public:
    MyDummyEvent(SkeletonBase& skeleton_base, const std::string_view event_name)
...
{
    auto& binding_mock_raw = SkeletonBaseView{skeleton}.GetBinding();
    auto* const mock_binding = dynamic_cast<mock_binding::Skeleton*>(&binding_mock_raw);
...
TEST_F(SkeletonBaseRegisterShmTracingFixture, RegisterShmObjectIsTracedIfTracingForSkeletonIsEnabled)
{
    ...
    ExpectIsTracePointEnabledCalls(
        expected_enabled_trace_points, kServiceIdentifier.ToString(), kDummyEventName, kInstanceSpecifier.ToString());
    ...
    // and that PrepareOffer will be called on the binding with the wrapped handler containing the register shm
    // object trace call
