...
class MyDummyField : public SkeletonFieldBase
{
  public:
    MyDummyField(SkeletonBase& /*skeleton_base*/,
                 ...
                 std::unique_ptr<SkeletonEventBinding> skeleton_event_base)
        : SkeletonFieldBase{
              field_name,
              std::make_unique<SkeletonEventBase>(field_name, std::nullopt, std::move(skeleton_event_base))}
    {
    }
  ...
  public:
    ...
    SkeletonTracingFixture()
        : empty_skeleton_{std::make_unique<mock_binding::Skeleton>(), kInstanceIdentifier},
          ...
          skeleton_event_base_{kEventName, std::nullopt, std::make_unique<mock_binding::SkeletonEvent>()},
          skeleton_field_base_{empty_skeleton_, kFieldName, std::make_unique<mock_binding::SkeletonEvent>()},
