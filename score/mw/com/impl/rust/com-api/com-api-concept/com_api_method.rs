...
pub trait MethodCaller<Args: MethodArgs, Return: CommData, R: Runtime + ?Sized> {
    ...
    ///
    /// Returns a `Result` containing the method return value if any otherwise unit or an error if the call failed.
...
// TODO: This can be expanded to the limit of function argument can have as per clippy lint.
// Currently, it supports up to 2 argument.
// Also this need to be done with declarative macro to avoid manual implementation.
impl MethodArgs for () {
    type PtrTuple = ();
}
...
impl<T1: CommData, T2: CommData, A: MethodInArgAllocator> MethodArgsAllocate<A> for (T1, T2) {
    ...
    fn alloc_uninit(allocator: &A) -> Self::UninitTuple {
        (allocator.allocate::<T1>(), allocator.allocate::<T2>())
    }
}
...
/// Zero-copy path for single-argument methods: pass a 1-tuple `(MethodInArgPtr<T>,)`.
/// This is consistent with the 2-arg zero-copy path which uses `(MethodInArgPtr<T1>, MethodInArgPtr<T2>)`
/// and allows a single declarative macro to generate all arity impls uniformly.
...
where
    F: Fn(T1) -> Return + Send + Sync + 'static,
{
    fn call(&self, args: (T1,)) -> Return {
        (self)(args.0)
    }
}
...
where
    F: Fn(T1, T2) -> Return + Send + Sync + 'static,
{
    fn call(&self, args: (T1, T2)) -> Return {
        (self)(args.0, args.1)
    }
}
