...
TEST_F(TransactionLogSetRegisterFixture, RegisterUnregisterMultipleTransactionLogsConcurrently)
{
    ...
    {
        threads.emplace_back([this, &unit, thread_number = TransactionLogIndex(i + 1U)]() noexcept {
            for (auto loop_count = 0U; loop_count < 50U; loop_count++)
            {
                auto register_result = unit->RegisterProxyElement(thread_number, proxy_event_data_control_local_);
