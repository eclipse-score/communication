...
TYPED_TEST(EventDataStorageTypedTest, CtorCallsInitializeSampleCallbackOnceForEverySlotWithDistinctSlotPointers)
{
    // Given a callback that records every pointer it was called with and default-constructs a TypeParam in-place via
    // placement-new (as a typed upper layer would do to initialize the type-erased storage)
