...
TYPED_TEST(EventDataStorageTypedTest, InitializeSlotsCallsCallbackOnceForEverySlotWithDistinctSlotPointers)
{
    // Given an EventDataStorage constructed for TypeParam (see fixture)

    // When calling InitializeSlots with a callback that records every pointer it was called with and
    // default-constructs a TypeParam in-place via placement-new (as a typed upper layer would do to initialize the
    // type-erased storage)
    std::vector<void*> received_slot_pointers{};
    this->unit_.InitializeSlots([&received_slot_pointers](void* const slot) noexcept {
        received_slot_pointers.push_back(slot);
        score::cpp::ignore = new (slot) TypeParam{};
    });

    // Then the callback was called exactly once for every slot
    ASSERT_EQ(received_slot_pointers.size(), kNumberOfSlots);
    ...
    for (SlotIndexType slot_index = 0U; slot_index < kNumberOfSlots; ++slot_index)
    {
        ...
        auto* const type_erased_slot = this->unit_.GetTypeErasedDataSlot(slot_index, sizeof(TypeParam));
        ...
        EXPECT_TRUE(BytesEqual(*static_cast<TypeParam*>(type_erased_slot), default_constructed_value));
    }
