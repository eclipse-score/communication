...
TEST_F(MethodCallRegistrationGuardFixture, MethodCallRegistrationGuardUsesMovableScopedOperation)
{
    // Expecting that MethodCallRegistrationGuard is a type alias for utils::MovableScopedOperation. If this is the
    ...
    // called when move constructing / move assigning the guard is handled in the tests for MovableScopedOperation.
    static_assert(std::is_same_v<MethodCallRegistrationGuard,
                                 utils::MovableScopedOperation<safecpp::MoveOnlyScopedFunction<void()>>>);
}
