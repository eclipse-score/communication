...
template <template <class> class AtomicIndirectorType>
score::cpp::optional<SlotIndexType> EventDataControlComposite<AtomicIndirectorType>::FindLatestReferencedSlotIndex(
...
bool EventDataControlComposite<AtomicIndirectorType>::TryIncreaseReferenceCount(ControlSlotType& slot) const noexcept
{
    ...
    const auto slot_new_value = static_cast<EventSlotStatus::value_type>(slot_new);
...
template <template <class> class AtomicIndirectorType>
auto EventDataControlComposite<AtomicIndirectorType>::GetLatestSlot(const QualityType& quality_type) const noexcept
    -> AllocationResult
{
    if (quality_type == QualityType::kASIL_B && asil_b_control_local_ == nullptr)
    {
        return {};
    }

    const ProviderEventDataControlLocalView<>& control =
        (quality_type == QualityType::kASIL_B) ? *asil_b_control_local_ : asil_qm_control_local_.get();

    for (std::size_t retry_counter{0U}; retry_counter < MAX_MULTI_ALLOCATE_RETRY_COUNT; ++retry_counter)
    {
        const auto latest_slot_index = FindLatestReferencedSlotIndex(control);
        ...
        auto& slot = control.state_slots_[slot_index];
        if (!TryIncreaseReferenceCount(slot))
        {
            continue;
        }
