...
/// constructing a ServiceDataControl (and its EventControls) for the given service_elements_size_info.
/// \details Mirrors the allocation sites accounted for by CalculateServiceDataControlShmSize: the ServiceDataControl
/// object itself, the event_controls_ and application_id_pid_mapping_ backing arrays (3 allocations, independent of
/// the number of service-elements), and, per service-element, the EventDataControl slots array, the
/// proxy_transaction_logs_ backing array, and one TransactionLog slots array per subscriber plus the fill-ctor
/// prototype and the skeleton-tracing log (see CalculateServiceDataControlShmSize for details).
std::size_t ComputeNumberOfAllocations(const std::vector<ServiceElementControlSizeInfo>& service_elements_size_info)
{
    std::size_t number_of_allocations{3U};
    for (const auto& service_element : service_elements_size_info)
    {
        number_of_allocations += 2U;
        number_of_allocations += service_element.max_subscribers + 2U;
    }
    return number_of_allocations;
}

/// \brief Asserts that calculated_size is a sufficient (i.e. not smaller than actual_allocated_bytes) but also not an
///        arbitrarily/excessively large over-estimation of the real allocation.
/// \details CalculateServiceDataControlShmSize rounds every individual allocation up to alignof(std::max_align_t),
///          whereas the real (monotonic) memory resource only pads as much as the current (running) offset actually
///          requires (between 0 and alignof(std::max_align_t) - 1 bytes). Hence the only legitimate source of waste
///          between the calculated and the actual size is this per-allocation alignment padding, bounded by
///          (alignof(std::max_align_t) - 1) * number_of_allocations. Should the calculation be egregiously wrong
///          (e.g. by a missing or wrongly-multiplied term), the resulting size would exceed this bound.
/// \attention If this expectation fails, check whether CalculateServiceDataControlShmSize implementation has changed
///            and handles padding/alignment differently!
void ExpectCalculatedSizeIsSufficientAndNotExcessive(
    const std::vector<ServiceElementControlSizeInfo>& service_elements_size_info,
...
TEST(ServiceDataControlShmSizeTest, EmptyServiceElementsYieldsSizeOfServiceDataControlWithoutEventControls)
{
    // Given no service-elements at all (an empty span)
    const score::cpp::span<const ServiceElementControlSizeInfo> empty_service_elements{};

    // When calculating the required shm-size for a ServiceDataControl
    const auto calculated_size = CalculateServiceDataControlShmSize(empty_service_elements);

    // Then the calculated size is sufficient for (i.e. greater than or equal to) the number of bytes actually
    // allocated when constructing a real ServiceDataControl with zero service-elements on the very same kind of
    ...
    const auto actual_allocated_bytes = ConstructServiceDataControlAndGetAllocatedBytes({}, resource);

    ExpectCalculatedSizeIsSufficientAndNotExcessive({}, calculated_size, actual_allocated_bytes);
}

TEST(ServiceDataControlShmSizeTest, SingleServiceElementCalculatedSizeIsSufficientForActualAllocation)
{
    // Given the sizing information of a single service-element (event/field)
    const std::vector<ServiceElementControlSizeInfo> service_elements_size_info{ServiceElementControlSizeInfo{5U, 3U}};

    // When calculating the required shm-size for a ServiceDataControl holding this single service-element
    ...
    // Then the calculated size is sufficient for (i.e. greater than or equal to) the number of bytes actually
    // allocated when constructing a real ServiceDataControl (and its single EventControl) with the very same sizing
    // information, and it does not exceed the actual allocation by more than the maximum possible per-allocation
    // alignment padding (see class-level documentation of CalculateServiceDataControlShmSize).
    memory::shared::NewDeleteDelegateMemoryResource resource{kMemoryResourceId};
...
TEST(ServiceDataControlShmSizeTest,
     MultipleServiceElementsWithDifferingSizesCalculatedSizeIsSufficientForActualAllocation)
{
    ...
    // Then the calculated size is sufficient for (i.e. greater than or equal to) the number of bytes actually
    // allocated when constructing a real ServiceDataControl (and its EventControls) with the very same sizing
    // information, and it does not exceed the actual allocation by more than the maximum possible per-allocation
    // alignment padding.
    memory::shared::NewDeleteDelegateMemoryResource resource{kMemoryResourceId};
    const auto actual_allocated_bytes =
