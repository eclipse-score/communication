...
void SkeletonEventCommon::EmplaceTransactionLogRegistrationGuard(TransactionLogSet* const transaction_log_set)
{
    ...
    SCORE_LANGUAGE_FUTURECPP_ASSERT_PRD_MESSAGE(transaction_log_set != nullptr,
                                                "TransactionLogSet pointer must be initialized if tracing is enabled.");
    score::cpp::ignore =
        transaction_log_registration_guard_.emplace(transaction_log_set->RegisterSkeletonTracingElement(
