...
class ProxyEvent final : public ProxyEventBase
{
  ...
  public:
    ...
    ProxyEvent(const std::string_view event_name, std::unique_ptr<ProxyEventBinding> proxy_event_binding);
  ...
  private:
    ProxyEventBinding* GetEventBinding() const noexcept;
...
ProxyEvent<SampleType>::ProxyEvent(const std::string_view event_name,
                                   std::unique_ptr<ProxyEventBinding> proxy_event_binding)
    : ProxyEventBase{event_name, std::unique_ptr{std::move(proxy_event_binding)}}, proxy_event_mock_{nullptr}
{
}
...
ProxyEvent<SampleType>::ProxyEvent(ProxyBase& base, const std::string_view event_name)
    ...
    : ProxyEventBase{event_name,
                     ProxyEventBindingFactory<SampleType>::Create(base.GetHandle(),
                         ...
                         .and_then([](std::unique_ptr<ProxyEventBinding> binding) {
...
Result<std::size_t> ProxyEvent<SampleType>::GetNewSamples(F&& receiver, std::size_t max_num_samples) noexcept
{
    ...
    tracing::TraceGetNewSamples(tracing_data_, *binding_);
    ...
    // The tracing_receiver created here always dispatches to the user provided receiver. Opposed to the user provided
    // receiver, which gets a typed sample (SamplePtr<SampleType>), it expects to receive type-erased sample#+
    // (SamplePtr<void>). Because this is what the type-erased binding layer provides. Before the tracing_receiver
    ...
    const auto get_new_samples_result = GetEventBinding()->GetNewSamples(std::move(tracing_receiver), guard_factory);
...
template <typename SampleType>
auto ProxyEvent<SampleType>::GetEventBinding() const noexcept -> ProxyEventBinding*
{
    if (binding_ == nullptr)
    {
        return nullptr;
...
class ProxyEventView
{
  public:
    ...
    ProxyEventBinding* GetBinding() const noexcept
    {
        return proxy_event_.GetEventBinding();
    }
