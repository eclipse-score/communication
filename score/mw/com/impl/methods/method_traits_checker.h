...
///
/// std::bind expressions make checking the callable signature in AssertCallableMatchesMethodSignature difficult. Since
/// the functionality of std::bind can be achieved using lambdas (which is even recommended over std::bind in general),
/// we disallow std::bind expressions as method handlers.
template <FailureMode FailureMode, typename Callable>
constexpr void AssertMethodCallableIsNotStdBind()
{
    CompileOrRuntimeAssert<!std::is_bind_expression_v<Callable>, FailureMode>(
        "std::bind expressions are not allowed as method handlers as they are not supported by "
        "AssertCallableMatchesMethodSignature which checks the callable signature. Please use a lambda instead.");
}
