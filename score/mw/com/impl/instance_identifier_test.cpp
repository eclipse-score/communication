...
TEST_F(InstanceIdentifierFixture, CreateStoresDeploymentsIntoConfiguredConfiguration)
{
    ...
    ASSERT_EQ(configuration.GetSizeOfServiceTypes(), 1);
    const auto type_deployment = configuration.FindServiceTypeDeployment(dummy_service);
    ...
    ASSERT_EQ(configuration.GetSizeOfServiceInstances(), 1);
    const auto instance_deployment = configuration.FindServiceInstanceDeployment(kInstanceSpecifier1);
