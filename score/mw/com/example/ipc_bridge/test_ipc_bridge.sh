...
function setup() {
    sudo useradd -g vscode mw_com_lola || true
    # run as mw_com_lola user via sudo without password
    echo "vscode ALL=(mw_com_lola) NOPASSWD:ALL" | sudo tee /etc/sudoers.d/vscode_mw_com_lola
    create_service_manifest "$MANIFEST_LOCATION" $(id -u) $(id -u mw_com_lola)
    trap "sudo deluser mw_com_lola && sudo rm -f /etc/sudoers.d/vscode_mw_com_lola $MANIFEST_LOCATION" EXIT
}
...
function run_receiver_sender() {
    ...
    sender_pid=$!
    sudo -u mw_com_lola $EXAMPLE_CMD_RECV --num-cycles 2 --mode recv > "$tempdir/recv.log" 2>&1
