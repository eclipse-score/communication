...
void run_get_provider(const score::cpp::stop_token& stop_token)
{
    ...
    // Step 2. Create skeleton
    std::cout << "\nProvider: Step 2 - Create skeleton" << std::endl;
    SkeletonContainer<GetterOnlySkeleton> skeleton_container{};
    skeleton_container.CreateSkeleton(kInstanceSpecifier, "get");
    auto& service = skeleton_container.GetSkeleton();

    // Step 3. Update field with initial value so the consumer's Get() reads it
    std::cout << "\nProvider: Step 3 - Update field with initial value (" << kInitialValue << ")" << std::endl;
    {
        const auto update_result = service.getter_only_field.Update(kInitialValue);
        if (!update_result.has_value())
        {
...
void run_get_and_notifier_provider(const score::cpp::stop_token& stop_token)
{
    ...
    // Step 3. Update field with initial value before offering so the consumer can
    //         subscribe, receive via notifier, and also call Get()
    std::cout << "\nProvider: Step 3 - Update field with initial value (" << kInitialValue << ")" << std::endl;
    {
...
void run_set_and_get_provider(const score::cpp::stop_token& stop_token)
{
    ...
    std::cout << "\nProvider: Step 3 - Register set handler (clamp to ["
              << kMinFieldValue << ", " << kMaxFieldValue << "])" << std::endl;
    service.set_and_get_enabled_field.RegisterSetHandler([](std::int32_t& value) noexcept {
        value = std::max(kMinFieldValue, std::min(kMaxFieldValue, value));
    });
...
void run_set_get_and_notifier_provider(const score::cpp::stop_token& stop_token)
{
    ...
    // Step 2. Create skeleton
    std::cout << "\nProvider: Step 2 - Create skeleton" << std::endl;
    SkeletonContainer<SetGetAndNotifierEnabledSkeleton> skeleton_container{};
    ...
    std::cout << "\nProvider: Step 3 - Register set handler (clamp to ["
              << kMinFieldValue << ", " << kMaxFieldValue << "])" << std::endl;
    service.set_get_and_notifier_enabled_field.RegisterSetHandler([](std::int32_t& value) noexcept {
        value = std::max(kMinFieldValue, std::min(kMaxFieldValue, value));
    });
    ...
    std::cout << "\nProvider: Step 4 - Update field with initial value (" << kInitialValue << ")" << std::endl;
    {
        const auto update_result = service.set_get_and_notifier_enabled_field.Update(kInitialValue);
        if (!update_result.has_value())
        {
            FailTest("Provider: Unable to update field with initial value: ", update_result.error());
        }
    ...
    // Step 7. Update field with new value so consumer can verify both Get() and GetNewSamples()
    std::cout << "\nProvider: Step 7 - Update field with updated value (" << kUpdatedValue << ")" << std::endl;
    {
        const auto update_result = service.set_get_and_notifier_enabled_field.Update(kUpdatedValue);
        if (!update_result.has_value())
        {
            FailTest("Provider: Unable to update field with updated value: ", update_result.error());
        }
...
std::optional<ProviderMode> ParseProviderMode(std::string_view mode)
{
    ...
    if (mode == "get")
    {
        return ProviderMode::kGet;
    }
    if (mode == "get_and_notifier")
    {
        return ProviderMode::kGetAndNotifier;
    }
    if (mode == "set_and_get")
    {
        return ProviderMode::kSetAndGet;
    }
    if (mode == "set_get_and_notifier")
    {
        return ProviderMode::kSetGetAndNotifier;
    }
    return std::nullopt;
}
...
void run_provider(const score::cpp::stop_token& stop_token, ProviderMode mode)
{
    switch (mode)
    {
        ...
        case ProviderMode::kSetAndNotifier:
            ...
            return;
        case ProviderMode::kGet:
            run_get_provider(stop_token);
            return;
        case ProviderMode::kGetAndNotifier:
            run_get_and_notifier_provider(stop_token);
            return;
        case ProviderMode::kSetAndGet:
            run_set_and_get_provider(stop_token);
            return;
        case ProviderMode::kSetGetAndNotifier:
            run_set_get_and_notifier_provider(stop_token);
            return;
    }
}
