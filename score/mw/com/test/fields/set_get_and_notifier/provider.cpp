...
#include "score/mw/com/test/fields/set_get_and_notifier/provider.h"
...
void run_provider(const score::cpp::stop_token& stop_token)
{
    ...
    SkeletonContainer<SetGetAndNotifierSkeleton> skeleton_container{};
    ...
    std::cout << "\nProvider: Step 3 - Register set handler (clamp to [" << kMinFieldValue << ", " << kMaxFieldValue
              << "])" << std::endl;
    service.set_get_and_notifier_enabled_field.RegisterSetHandler(
        [](const std::int32_t& requested_value) -> std::int32_t {
            return std::max(kMinFieldValue, std::min(kMaxFieldValue, requested_value));
        });

    // Step 4. Update field with initial value (this also triggers the initial subscription notification)
    ...
    // Step 7. Update field with new value so consumer can verify via both Get() and GetNewSamples()
