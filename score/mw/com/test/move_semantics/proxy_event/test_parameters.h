...
/// Shared-memory paths used by ProcessSynchronizer to coordinate provider and consumer. Defined here
/// (rather than in anonymous namespaces inside provider.cpp / consumer.cpp) so that main_consumer_and_provider.cpp
/// can pre-create the SHM objects before launching the two async tasks, eliminating the startup race where
/// both threads simultaneously compete for the same LockFile via CreateOrOpenObject.
const std::string kInterprocessNotificationShmPath{
    "/proxy_event_move_semantics_interprocess_notificationMoveEventInterfaceMovedTo"};
const std::string kProviderWithdrawNotificationShmPath{
    "/proxy_event_move_semantics_provider_withdraw_notificationMoveEventInterfaceMovedTo"};
