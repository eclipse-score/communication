...
void PerformProviderActions(CheckPointControl& check_point_control, std::string_view mw_com_config_path, score::cpp::stop_token stop_token)
{
    ...
    while (!stop_token.stop_requested())
    {
        ...
        if (!result.has_value())
        {
            std::cout << "Provider Step(2): Sending of event failed: " << result.error() << std::endl;
