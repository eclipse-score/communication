                       ...
                       std::string_view service_instance_manifest) noexcept
{
    ...
    {
        std::cout << "Consumer Step (1): Did not receive handle in time!" << std::endl;
        return;
    }
    ...
    {
        std::cout << "Consumer: Unable to create lola proxy:" << lola_proxy_result.error() << "\n";
        return;
    }
    std::cout << "Consumer: Successfully created lola proxy (with methods)" << std::endl;
    ...
    for (std::size_t call_index = 0U; call_index < method_call_count; ++call_index)
    {
        ...
        if (!result.has_value())
        {
            std::cout << "Consumer: Method call failed with error: " << result.error().Message() << std::endl;
            return;
        }
        ...
        {
            std::cout << "Consumer: Method result mismatch! Got " << *result.value() << " but expected "
                      << expected_result << std::endl;
            return;
        }
    }

    std::cout << "Consumer: All method calls succeeded - checkpoint (1) reached!" << std::endl;
    ...
    // ********************************************************************************
    std::cout << "Consumer Step (4): Wait for controller notification to finish" << std::endl;
    ...
    {
        FailTest("Consumer Step (5): Stop async FindService failed with error: ", stop_find_service_result.error());
    }

    check_point_control_error_guard.Release();
    std::cout << "Consumer: Exiting gracefully." << std::endl;
}
