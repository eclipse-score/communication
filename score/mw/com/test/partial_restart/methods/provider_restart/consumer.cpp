...
constexpr std::uint8_t kCheckpointInitialMethodCallsSucceeded{1U};
constexpr std::uint8_t kCheckpointServiceDisappeared{2U};
constexpr std::uint8_t kCheckpointServiceReappeared{3U};
constexpr std::uint8_t kCheckpointMethodCallsAfterRestartSucceeded{4U};
                                ...
                                const ConsumerParameters&) noexcept
{
    ...
    // ********************************************************************************
    std::cerr << "Consumer: All initial method calls succeeded - checkpoint (1) reached!" << std::endl;
    check_point_control.CheckPointReached(kCheckpointInitialMethodCallsSucceeded);
    ...
    // ********************************************************************************
    std::cerr << "Consumer: Now waiting for service to disappear!" << std::endl;
    WaitTillServiceDisappears(handle_notification_data);

    std::cerr << "Consumer: Service disappeared - checkpoint (2) reached!" << std::endl;
    check_point_control.CheckPointReached(kCheckpointServiceDisappeared);
    ...
    // ********************************************************************************
    std::cerr << "Consumer: Now waiting for service to reappear!" << std::endl;
    if (!WaitTillServiceAppears(handle_notification_data, kMaxHandleNotificationWaitTime))
    {
        std::cerr << "Consumer: Did not receive handle in time after provider restart!" << std::endl;
        check_point_control.ErrorOccurred();
        return;
    }
    ...
    for (std::size_t call_index = 0U; call_index < method_call_count;)
    {
        ...
        auto call_method_until_success = [&method_proxy, &arg1, &arg2]() -> auto {
            ...
            // clang-format off
            fail_test("Consumer: Method call after restart failed. A retry was attempted", call_retry_count, " times. ",
        ...
        if (*result.value() != expected_result)
        {
            std::cerr << "Consumer: Method result mismatch after restart! Got " << *result.value() << " but expected "
                      << expected_result << std::endl;
            check_point_control.ErrorOccurred();
            return;
        }
    ...
    // ********************************************************************************
    std::cerr << "Consumer: All method calls after provider restart succeeded - checkpoint (4) reached!" << std::endl;
    check_point_control.CheckPointReached(kCheckpointMethodCallsAfterRestartSucceeded);
                       ...
                       const ConsumerParameters& test_params) noexcept
{
    ...
    auto instance_specifier_result =
        score::mw::com::InstanceSpecifier::Create(std::string{"partial_restart/methods/provider_restart"});
    ...
    if (test_params.with_proxy)
    {
        DoConsumerActionsWithProxy(check_point_control, handle_notification_data, test_stop_token, test_params);
    }
    else
    {
        // For tests without proxy, just monitor service availability
        std::cerr << "Consumer: Service found - checkpoint (1) reached!" << std::endl;
        check_point_control.CheckPointReached(kCheckpointInitialMethodCallsSucceeded);

        auto wait_for_child_proceed_result = WaitForChildProceed(check_point_control, test_stop_token);
        if (wait_for_child_proceed_result != CheckPointControl::ProceedInstruction::PROCEED_NEXT_CHECKPOINT)
        {
            std::cerr << "Consumer: Expected to get notification to continue but got: "
                      << static_cast<std::uint8_t>(wait_for_child_proceed_result) << std::endl;
            check_point_control.ErrorOccurred();
            return;
        }
