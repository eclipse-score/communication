...
std::optional<TestParameters> ParseTestParameters(int argc, const char** argv) noexcept
{
    ...
    options.add_options()
        ...
        ("with-proxy", po::value<bool>(&test_parameters.with_proxy), "Shall a proxy instance be created from any found service handle?");
    // clang-format on
...
int main(int argc, const char** argv)
{
    ...
    for (std::size_t test_iteration = 1U; test_iteration <= test_parameters.number_test_iterations; ++test_iteration)
    {
        ...
        if (test_parameters.kill_provider == false)
        {
            ...
            {
                test_result = score::mw::com::test::DoProviderNormalRestartWithProxy(
                    test_stop_source.get_token(), mw_com_argc, mw_com_argv);
            }
            else
            {
                test_result = score::mw::com::test::DoProviderNormalRestartWithoutProxy(
                    test_stop_source.get_token(), mw_com_argc, mw_com_argv);
            }
        }
        else
        {
            if (test_parameters.with_proxy)
            {
                test_result = score::mw::com::test::DoProviderKillRestartWithProxy(
                    test_stop_source.get_token(), mw_com_argc, mw_com_argv);
            }
            else
            {
                test_result = score::mw::com::test::DoProviderKillRestartWithoutProxy(
                    test_stop_source.get_token(), mw_com_argc, mw_com_argv);
            }
        }
        ...
        if (test_result != EXIT_SUCCESS)
        {
            std::cerr << "Test Main: Iteration " << test_iteration << " of " << test_parameters.number_test_iterations
                      << " of Provider-Restart-Methods-Test failed. Skipping any further iteration." << std::endl;
            break;
        }
    }
