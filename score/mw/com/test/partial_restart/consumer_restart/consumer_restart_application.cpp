...
namespace
{
    ...
    const auto kShmProviderCheckpointControlFileName = "consumer_restart_application_provider_checkpoint_file";
    const auto kShmConsumerCheckpointControlFileName = "consumer_restart_application_consumer_checkpoint_file";
    ...
    amp::optional<TestParameters> ParseTestParameters(int argc, const char **argv) noexcept
    {
    ...
    int DoConsumerRestart(amp::stop_token test_stop_token, int argc, const char **argv, bool kill_consumer) noexcept
    {
        // Resources that need to be cleaned up on process exit
        bmw::mw::com::test::ObjectCleanupGuard object_cleanup_guard{};
        ...
        // exiting.
        auto consumer_checkpoint_control_guard_result = bmw::mw::com::test::CreateSharedCheckPointControl(
        ...
        auto fork_consumer_pid_guard = bmw::mw::com::test::ForkProcessAndRunInChildProcess(
            ...
            {
                const bmw::mw::com::test::ConsumerParameters consumer_paramaters{kill_consumer};
        ...
        auto provider_checkpoint_control_guard_result = bmw::mw::com::test::CreateSharedCheckPointControl(
        ...
        {
            // ********************************************************************************
            // Step (4.2) - Trigger consumer to finish (consumer will terminate gracefully now)
            // ********************************************************************************
            std::cout << "Controller Step (4.2): Trigger consumer to finish" << std::endl;
            consumer_checkpoint_control.FinishActions();

            // ********************************************************************************
            // Step (4.3) - Wait for consumer process to finish
            // ********************************************************************************
            std::cout << "Controller Step (4.3): Waiting for consumer to finish" << std::endl;
            const auto consumer_terminated = WaitForChildProcessToTerminate(
                "Controller Step (4.3)", fork_consumer_pid_guard.value(), kMaxWaitTimeToReachCheckpoint);
        ...
        // gracefully.
        // ********************************************************************************
        std::cout << "Controller Step (5): Re-forking provider process" << std::endl;
        fork_consumer_pid_guard = bmw::mw::com::test::ForkProcessAndRunInChildProcess(
            ...
            const bmw::mw::com::test::ConsumerParameters consumer_paramaters{kill_consumer_after_restart};
...
int main(int argc, const char **argv)
{
    ...
    amp::set_assertion_handler(&bmw::mw::com::test::assertion_stdout_handler);
