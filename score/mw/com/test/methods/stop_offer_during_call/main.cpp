...
using score::mw::com::MethodReturnTypePtr;
using score::mw::com::test::FailTest;
using score::mw::com::test::StopOfferDuringCallProxy;
using score::mw::com::test::StopOfferDuringCallSkeleton;
                  ...
                  std::int32_t expected_value)
{
    ...
    // clang-format off
    if (*return_value_ptr != expected_value)
    {
        FailTest("FAIL: Method call returned wrong value. Expected: ", expected_value, ", Actual: ", *return_value_ptr);
    }
    // clang-format on
...
int main(int argc, const char** argv)
{
    ...
    std::cout << "Step 2.3: Service offered" << std::endl;

    std::cout << "Step 3: Creating proxy" << std::endl;
    ...
    std::cout << "Step 3.1: Call the long running method asynchronously\n";
    ...
    while (repetition_count < max_repetition_count)
    {
        ...
        std::cout << "Step 4.1: Wait for handler to start, then call StopOfferService\n" << std::flush;
        ...
        {
            std::cout << "Step 4.2: StopOfferService was not called since the method was already completed\n"
                      << std::flush;
            continue;
        }
        ...
        std::cout << "Step 4.3: StopOfferService was called while the method was still running\n" << std::flush;
