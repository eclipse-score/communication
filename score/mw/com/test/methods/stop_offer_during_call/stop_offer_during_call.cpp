                  ...
                  std::int32_t expected_value)
{
    ...
    // fail_test_if(!result.has_value(), "FAIL: Method call failed with error unexpectedly: ", result.error());
    if (!result.has_value())
    {
        fail_test("FAIL: Method call failed with error unexpectedly: ", result.error());
    }

    auto& return_value_ptr = result.value();

    fail_test_if(return_value_ptr.get() == nullptr,
                 " FAIL: Method call returned null pointer, instead of a valid ptr to shm.");

    // clang-format off
    fail_test_if(*return_value_ptr != expected_value,
...
void offer_service(StopOfferDuringCallSkeleton& skeleton)
{
    auto offer_result = skeleton.OfferService();
    if (!offer_result.has_value())
    {
        fail_test("Could not offer service: ", offer_result.error().Message());
    }
    std::cout << "Service offered successfully\n";
}

auto find_service(InstanceSpecifier& instance_specifier) -> StopOfferDuringCallProxy
{

    auto find_result = StopOfferDuringCallProxy::FindService(instance_specifier);
    if (!find_result.has_value() || find_result->size() != 1)
    {
        fail_test("FindService failed or returned wrong count. Error: ", find_result.error());
    }

    auto proxy_result = StopOfferDuringCallProxy::Create((*find_result)[0], {"long_running_method"});
    if (!proxy_result.has_value())
    {
        fail_test("Failed to create proxy: ", proxy_result.error());
    }
...
int main(int argc, const char** argv)
{
    ...
    std::cout << "Step 1.2: Creating a Proxy and finding reoffered service" << std::endl;
    auto proxy = find_service(instance_specifier);
    ...
    while (repetiction_count < max_repetition_count)
    {
        ...
        std::cout << "Step 3.1 : (Re)Offering the service\n" << std::flush;
        ...
        std::cout << "Step 3.2 : Wait for handler to start, then call StopOfferService\n" << std::flush;
        ...
        {
            std::cout << " Step 3.3: StopOfferService was not calld since the method was already completed\n"
                      << std::flush;
            continue;
        }
        ...
        std::cout << " Step 3.4: StopOfferService was called while the method was still running\n" << std::flush;
