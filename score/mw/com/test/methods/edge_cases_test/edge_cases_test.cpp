...
void TestDisabledMethodReturnsError()
{
    ...
    {
        fail_test("Failed to create skeleton: ", skeleton_result.error());
    }
    ...
    {
        fail_test("Failed to offer service: ", offer_result.error());
    }
    ...
    {
        fail_test("Failed to create proxy");
    }
    ...
    {
        fail_test("Enabled method_with_args_and_return failed: ", result_args_return.error());
    }
    ...
    {
        fail_test("Enabled method_with_args_only failed: ", result_args_only.error());
    }
    ...
    {
        fail_test("Enabled method_with_return_only failed: ", result_return_only.error());
    }
    ...
    {
        fail_test("ERROR: Disabled method succeeded when it should have returned error!", disabled_result.error());
    }
...
void TestIncompleteHandlers()
{
    ...
    if (offer_result.has_value())
    {
        fail_test("ERROR: OfferService succeeded when it should have failed!");
        skeleton->StopOfferService();
    }
...
void TestProxyRecreation()
{
    ...
    // Create first proxy, use it, then destroy it
    {
        ...
        fail_test_if(!proxy_container1.CreateProxy(kInstanceSpecifier, kAllMethodNames),
        ...
        {
            fail_test("First proxy method call failed: ", result1.error());
        }
        fail_test_if(*result1.value() != kExpectedSum, "First proxy returned wrong value");
    ...
    // Create second proxy after first is destroyed
    {
        ...
        fail_test_if(!proxy_container2.CreateProxy(kInstanceSpecifier, kAllMethodNames),
        ...
        {
            fail_test("Second proxy method call failed: ", result2.error());
        }
        fail_test_if(*result2.value() != kExpectedSum, "Second proxy returned wrong value");
...
void TestSkeletonRecreation()
{
    ...
    // --- First skeleton lifecycle ---
    {
        ...
        if (!skeleton_result.has_value())
        {
            fail_test("Failed to create first skeleton: ", skeleton_result.error());
        }
        auto skeleton = std::make_unique<EdgeCasesMethodSkeleton>(std::move(skeleton_result).value());
        ...
        if (!offer_result.has_value())
        {
            fail_test("First OfferService failed: ", offer_result.error());
        }

        // Create proxy and verify methods work with first skeleton
        {
            ProxyContainer<EdgeCasesMethodProxy> proxy_container;
            fail_test_if(!proxy_container.CreateProxy(kInstanceSpecifier, kAllMethodNames),
            ...
            if (!result.has_value())
            {
                fail_test("First skeleton method call failed: ", result.error());
            }
            fail_test_if(*result.value() != kExpectedSum, "First skeleton returned wrong value");
    ...
    // --- Second skeleton lifecycle (recreation) ---
    {
        ...
        if (!skeleton_result.has_value())
        {
            fail_test("Failed to create second skeleton: ", skeleton_result.error());
        }
        auto skeleton = std::make_unique<EdgeCasesMethodSkeleton>(std::move(skeleton_result).value());
        ...
        if (!offer_result.has_value())
        {
            fail_test("Second OfferService failed: ", offer_result.error());
        }
        ...
        // Create proxy and verify methods work with second skeleton
        {
            ...
            if (!result.has_value())
            {
                fail_test("Second skeleton method call failed: ", result.error());
            }
            fail_test_if(*result.value() != kExpectedSum, "Second skeleton returned wrong value");
...
auto GetTestType(int argc, const char** argv) -> TestType
{
    ...
    fail_test_if(!test_type_str.has_value(), "No test type specified");
