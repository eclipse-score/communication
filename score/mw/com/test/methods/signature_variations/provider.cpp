...
{
    MethodProvider<MethodSignatureSkeleton> provider{};
    auto process_synchronizer_result = ProcessSynchronizer::Create(kInterprocessNotificationShmPath);
    if (!(process_synchronizer_result.has_value()))
    {
        std::cerr << "Methods signature_variations provider failed: Could not create ProcessSynchronizer" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << "Methods signature_variations provider failed: CreateSkeleton" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << "Methods signature_variations provider failed: RegisterMethodHandlerWithInArgsAndReturn"
                  << std::endl;
        return EXIT_FAILURE;
    }
    if (!provider.RegisterMethodHandlerWithInArgsOnly(kInArgOnlyMethodTestValueA, kInArgOnlyMethodTestValueB))
    {
        std::cerr << "Methods signature_variations provider failed: RegisterMethodHandlerWithInArgsOnly" << std::endl;
        return EXIT_FAILURE;
    }
    if (!provider.RegisterMethodHandlerWithReturnOnly(kReturnOnlyMethodReturnValue))
    {
        std::cerr << "Methods signature_variations provider failed: RegisterMethodHandlerWithReturnOnly" << std::endl;
        return EXIT_FAILURE;
    }
    if (!provider.RegisterWithoutInArgsOrReturn())
    {
        std::cerr << "Methods signature_variations provider failed: RegisterWithoutInArgsOrReturn" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << "Methods signature_variations provider failed: OfferService" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << "Methods signature_variations provider failed: WaitForProxyTestToFinish was stopped by "
