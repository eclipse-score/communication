    ...
    -> std::vector<std::string_view>
{
    ...
    {
        // clang-format off
        fail_test_if(method_id > method_names.size(), "Trying to create more methods than allowed", method_id, " is larger than ", method_names.size());
        // clang-format on
                              ...
                              std::int32_t input_value)
{
    if (!result.has_value())
    {
        fail_test("Consumer", consumer_id, ": method", method_id, " call failed: ", result.error().Message());
    }
...
void CallMethod(Proxy& proxy, CopyMode copy_mode, std::size_t method_id, std::size_t consumer_id, std::int32_t input)
{
    ...
    auto& method = [&proxy, consumer_id, method_id]() -> auto& {
        ...
        // clang-format off
        fail_test("Consumer", consumer_id, ": Invalid method ID ", method_id, " can not be larger than ", kMaxRegisteredMethos);
        // clang-format on
...
auto ReadCommandLineArguments(int argc, const char** argv) -> ConsumerTestConfiguration
{
    ...
    {
        fail_test("Consumer: --consumer_id parameter is not optional.\n");
    }
    test_configuration.consumer_id = consumer_id_opt.value();
    if (test_configuration.consumer_id > 3)
    {
        fail_test("Consumer: consumer_id value ", test_configuration.consumer_id, " is invalid (must be between 0-2)");
    }
    ...
    if (num_proxies_per_process_opt.has_value())
    {
        ...
        {
            fail_test("Consumer: num-proxies-per-consumer value ",
                  ...
                  const std::vector<std::size_t>& enabled_method_ids)
{
    ...
    {
        fail_test("Consumer", test_configuration.consumer_id, ": Could not create the ProcessSynchronizer.\n");
    }
    ...
    auto proxy_runner = [&enabled_method_ids, &method_names](const ConsumerTestConfiguration& test_configuration,
        ...
        // clang-format off
        fail_test_if(!instance_specifier_result.has_value(),
        ...
        {
            fail_test("Consumer", test_configuration.consumer_id, ": Could not create the proxy with ID ", proxy_id);
        }
