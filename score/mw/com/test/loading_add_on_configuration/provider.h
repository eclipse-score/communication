...
#ifndef SCORE_MW_COM_TEST_LOADING_ADD_ON_CONFIGURATION_PROVIDER_H
#define SCORE_MW_COM_TEST_LOADING_ADD_ON_CONFIGURATION_PROVIDER_H
...
{
template <typename T, typename SkeletonType>
                  ...
                  std::function<void()> mid_stream_callback = {})
{
         ...
         ++cycle)
    {
        ...
        // Invoke the callback (if provided) once, roughly halfway through sending the samples, so that
        // callers can perform an action (e.g. merging an add-on configuration) while the service is actively
        // streaming to a subscribed consumer.
        if (mid_stream_callback && cycle == samples.size() / 2U)
        {
            std::invoke(mid_stream_callback);
        }
...
#endif  // SCORE_MW_COM_TEST_LOADING_ADD_ON_CONFIGURATION_PROVIDER_H
