...
namespace bmw::mw::com::test
{
    ...
    CheckPointControl::CheckPointControl(const amp::string_view check_point_owner_name) noexcept
    ...
    bool CheckPointControl::WaitForProceedOrFinishTrigger(amp::stop_token stop_token) noexcept
    {
                          ...
                          std::uint8_t expected_check_point)
    {
        ...
        {
            std::cerr << tag << checkpoint_control.GetOwnerName().data() << " failed: didn't reach checkpoint in time!"
                      << std::endl;
            return false;
        }
        ...
        {
            std::cerr << tag << ": " << checkpoint_control.GetOwnerName().data() << " failed: reported Error." << std::endl;
            return false;
        }
        if (checkpoint_control.GetReachedCheckPoint() != expected_check_point)
        {
            std::cerr << tag << ": " << checkpoint_control.GetOwnerName().data()
                      << " failed: reached unexpected checkpoint " << checkpoint_control.GetReachedCheckPoint()
                      << ". Expected: " << static_cast<int>(expected_check_point) << std::endl;
            return false;
        }
        std::cout << tag << ": " << checkpoint_control.GetOwnerName().data() << " reached checkpoint "
                                ...
                                std::chrono::milliseconds wait_duration)
    {
        ...
        std::cout << tag << "wait till check point is reached\n";
