...
bool DiscoverIvshmemBar(std::uint64_t& paddr, std::uint64_t& size, std::uint32_t required_bar_num) noexcept
{
    ...
    {
        ::score::mw::log::LogError() << "ThreadCtl(_NTO_TCTL_IO) failed: " << std::string{std::strerror(errno)};
        return false;
    }
    ...
    const pci_devhdl_t hdl = pci_device_attach(bdf, pci_attachFlags_MULTI_OWNER, &err);
    ...
    {
        if (ba[i].type == pci_asType_e_MEM && static_cast<std::uint32_t>(ba[i].bar_num) == required_bar_num)
        {
            paddr = static_cast<std::uint64_t>(ba[i].addr);
            size = static_cast<std::uint64_t>(ba[i].size);
            found = true;
            break;
        }
    }
    ...
    if (!found)
    {
        for (int_t i = 0; i < nba; ++i)
        {
            ::score::mw::log::LogInfo() << "ivshmem: BAR" << static_cast<int>(ba[i].bar_num)
                                        << " type=" << std::string{ba[i].type == pci_asType_e_MEM ? "MEM" : "IO"}
                                        << " addr=0x" << static_cast<std::uint64_t>(ba[i].addr) << " size=0x"
    ...
    ::score::mw::log::LogInfo() << "ivshmem: BAR" << static_cast<unsigned int>(required_bar_num) << " paddr=0x" << paddr
                                << " size=0x" << size;
    return true;
}
// COV_JUSTIFIED_STOP
