...
std::optional<std::uint64_t> LookupOffsetInLockedDirectory(const std::string& shm_name,
...
#if defined(__QNXNTO__)
score::cpp::expected_blank<score::os::Error> IvshmemTypedMemoryProvider::WriteDirectoryEntry(
    ...
    std::uint32_t size) const noexcept
{
    ...
    if (count >= kMaxDirectoryEntries)
    {
        ...
        return score::cpp::make_unexpected(score::os::Error::createFromErrno(ENOSPC));
    }
    ...
    const std::string& shm_name) const noexcept
{
    ...
    return LookupOffsetInLockedDirectory(shm_name, header);
#else
    (void)shm_name;
    return std::nullopt;
#endif
    ...
    const score::memory::shared::permission::UserPermissions& /*permissions*/) const noexcept
{
    ...
    auto dir_offset = LookupOffsetInLockedDirectory(shm_name, header);
    ...
    const auto directory_result = WriteDirectoryEntry(shm_name, offset, static_cast<std::uint32_t>(alloc_size));
