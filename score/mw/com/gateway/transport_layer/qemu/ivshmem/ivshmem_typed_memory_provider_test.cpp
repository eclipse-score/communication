...
TEST_F(IvshmemTypedMemoryProviderTest, AllocateNamedTypedMemoryReturnsEnosysOnNonQnx)
{
    ...
    // Then the stub returns ENOSYS
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), ENOSYS);
}
...
TEST(IvshmemTypedMemoryProviderBindShmToBarTest, ReturnsErrorWhenShmOpenFails)
{
    ...
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), ENOMEM);
}
...
TEST(IvshmemTypedMemoryProviderBindShmToBarTest, ReturnsErrorWhenShmCtlFails)
{
    ...
    // Then the operation fails
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), EINVAL);
}
...
TEST_F(IvshmemTypedMemoryProviderTest, UnlinkReturnsErrorWhenShmUnlinkFails)
{
    ...
    // Then the operation fails with ENOENT
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), ENOENT);
}
...
TEST(IvshmemQnxAllocateNamedTest, ReturnsErrorWhenBarIsExhausted)
{
...
TEST(IvshmemQnxAllocateNamedTest, FullDirectoryReturnsEnospcBeforeBinding)
{
    ...
    // A full directory must prevent binding an allocation that the other VM cannot discover.
    EXPECT_CALL(*raw,
                mmap(nullptr,
                     IvshmemTypedMemoryProvider::kDirectorySize,
                     ::testing::_,
    ...
    // Then allocation fails before creating a local shm object
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), ENOSPC);
}

TEST(IvshmemQnxAllocateAtOffsetTest, ReturnsErrorWhenBarIsExhausted)
{
    ...
    // Then allocation fails with ENOMEM
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), ENOMEM);
}
...
TEST(IvshmemQnxAllocateAtOffsetTest, ReturnsEexistWhenOffsetMismatch)
{
    ...
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error().GetOsDependentErrorCode(), EEXIST);
}
