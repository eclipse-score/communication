    ...
    : public common::InstantiationStrategyBase<MwComProxy, internal::ProxyHolderCreator<MwComProxy>>
{
  ...
  public:
    ...
    {
        Base::StartFind([on_found{std::move(on_found)}](std::vector<std::unique_ptr<MwComProxy>> ara_proxies,
                                                        std::string_view /*port_identifier*/) ->
                        typename Base::ShallStopFindService {
                            for (auto& ara_proxy : ara_proxies)  // LCOV_EXCL_BR_LINE (tooling issue Ticket-188259)
                            {
                                std::invoke(*on_found, std::make_unique<ProxyImpl>(std::move(ara_proxy)));
                            }
