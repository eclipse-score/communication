...
Reads LCOV coverage data, clang-tidy findings, and CodeQL/MISRA findings (the
deduplicated Linux ∪ QNX union CSV, generated by
//quality/static_analysis:merge_codeql_findings from the merged SARIF using
sarif-tools), then renders a dark-themed HTML summary page via the
dashboard.html.j2 Jinja2 template.
...
Usage (CI, called from nightly_quality.yml):
    bazel run //quality/dashboard:generate_dashboard -- \\
        ...
        --codeql-csv           _quality/codeql_findings.txt \\
...
_CODEQL_FIELDS = ("Tool", "Severity", "Code", "Description", "Location", "Line")


def _codeql_row_fields(row: dict) -> dict:
    """Extract the fixed fields of one CodeQL CSV row.

    The CSV is generated by //quality/static_analysis:merge_codeql_findings via
    sarif-tools' CSV exporter, so it always uses sarif-tools' own record schema.
    """
    if set(row.keys()) != set(_CODEQL_FIELDS):
        raise ValueError(
            f"Unexpected CodeQL CSV schema: {sorted(row.keys())!r} "
            f"(expected {list(_CODEQL_FIELDS)!r})"
        )
...
def load_codeql_csv(path: pathlib.Path) -> dict | None:
    """Return CodeQL summary counts for the deduplicated Linux ∪ QNX union CSV."""
    if not path or not path.is_file():
        return None
    ...
    try:
        with path.open(encoding="utf-8", errors="replace", newline="") as fh:
            ...
            if reader.fieldnames != list(_CODEQL_FIELDS):
                raise ValueError(
                    f"Unexpected CodeQL CSV schema in {path}: {reader.fieldnames!r} "
                    f"(expected {list(_CODEQL_FIELDS)!r})"
                )
            for row in reader:
                fields = _codeql_row_fields(row)
        ...
        raise ValueError(f"Error parsing CodeQL CSV {path}: {e}") from e
...
def write_github_summary(cov_summary, clang_tidy, clippy, codeql, history, summary_path) -> None:
    ...
    if codeql:
        ...
        lines.append(
            "\n_Counts summarize the deduplicated Linux \u222a QNX union._"
        )
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        "--codeql-csv", default="",
        dest="codeql_csv",
        help="Path to CodeQL CSV results file (the deduplicated Linux ∪ QNX union)",
    )
    parser.add_argument(
