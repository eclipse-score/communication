...
IMG=${IMG:-bazel-bin/quality/integration_testing/environments/dual_qemu/example/init_ifs_test_qemu_shared_ivshmem}
...
if [[ ! -f "${IMG}" ]]; then
    ...
    echo "  bazel build --config=qnx_x86_64 //quality/integration_testing/environments/dual_qemu/example:_init_ifs_test_qemu_shared_ivshmem" >&2
    exit 1
fi
